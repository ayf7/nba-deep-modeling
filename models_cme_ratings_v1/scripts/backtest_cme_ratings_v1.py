#!/usr/bin/env python3
"""Expanding-window monthly backtest for CME-Ratings-v1.
...
from cme_ratings_v1_model import CmeRatingsV1, CmeRatingsV1Config  # noqa: E402
from train_cme_ratings_v1 import (  # noqa: E402
...
def predict_window(
    model: CmeRatingsV1, records: list, batch_size: int, device: str,
) -> tuple[np.ndarray, ...]:
    model.eval()
    loader = DataLoader(GameDatasetRatingsV1(records), batch_size=batch_size,
    ...
    rating_logit_adjustments: list[np.ndarray] = []
    rating_feature_residuals: list[np.ndarray] = []
    rating_logit_weights: list[np.ndarray] = []
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            rating_logit_adjustments.append(out["rating_logit_adjustment"].cpu().numpy())
            rating_feature_residuals.append(out["rating_feature_residual"].cpu().numpy())
            rating_logit_weights.append(np.full(probs_arr.shape, out["rating_logit_weight"].detach().cpu().item()))
    ...
    return (
        ...
        np.concatenate(rating_logit_adjustments),
        np.concatenate(rating_feature_residuals),
        np.concatenate(rating_logit_weights),
...
def _build_ckpt_payload(model, cfg, vocab, team_vocab, box_weights, pair_weights,
                        ...
                        rating_feature_reg_w, rating_weight_reg_w, rating_feature_stats, epoch):
    return {
        ...
        "loss_level_weights": {
            ...
            "rating_feature_reg": rating_feature_reg_w,
            "rating_weight_reg": rating_weight_reg_w,
        },
        "epoch": epoch,
    }


def _build_optimizer(model: CmeRatingsV1, args) -> torch.optim.Optimizer:
...
def _inspect_player_predictions(
    ...
    loader = DataLoader(GameDatasetRatingsV1(sample), batch_size=len(sample),
                        shuffle=False, collate_fn=collate_ratings_v1)
    pts_i = BOX_INDEX["pts"]
    fgm_i = BOX_INDEX["fgm"]
    fga_i = BOX_INDEX["fga"]
...
def _track_player_predictions(
    ...
    loader = DataLoader(GameDatasetRatingsV1(test_recs), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_ratings_v1)
    model.eval()
...
def train_one_window(
    ...
    if args.curriculum:
        ...
        ckpt_rating_feature_reg_w = args.rating_feature_reg_w
        ckpt_rating_weight_reg_w = args.rating_weight_reg_w
    ...
    if ckpt_dir is not None and args.save_final:
        final_payload = _build_ckpt_payload(
            ...
            ckpt_rating_feature_reg_w, ckpt_rating_weight_reg_w, rating_feature_stats, last_epoch,
        )
    else:
        final_payload = None
    ...
    if ckpt_dir is not None:
        ...
        torch.save(
            _build_ckpt_payload(
                ...
                ckpt_team_w, ckpt_player_w, ckpt_pair_w, ckpt_inv_w, ckpt_win_w,
                ckpt_margin_nll_w, ckpt_calibration_reg_w, ckpt_season_calibration_reg_w, ckpt_calibration_slope_reg_w,
                ckpt_rating_feature_reg_w, ckpt_rating_weight_reg_w, rating_feature_stats, best_epoch,
            ),
            ckpt_dir / "best.pt",
        )
    ...
    (gids, probs, base_probs, affine_probs, temporal_probs, season_logit_adjustments,
     ...
     rating_logit_adjustments, rating_feature_residuals, rating_logit_weights,
    ...
    return (model, window_info, gids, probs, base_probs, affine_probs, temporal_probs,
            ...
            rating_margin_priors, rating_logit_priors, rating_logit_adjustments,
            rating_feature_residuals, rating_logit_weights,
...
def main() -> None:
    ...
    for wi, window_start in enumerate(windows):
        ...
        rating_logit_adjustments_ordered = rating_logit_adjustments[order]
        rating_feature_residuals_ordered = rating_feature_residuals[order]
        rating_logit_weights_ordered = rating_logit_weights[order]
        ...
        win_metrics = {
            ...
            "test_mean_rating_logit_adjustment": float(np.mean(rating_logit_adjustments_ordered)),
            "test_mean_rating_feature_residual": float(np.mean(rating_feature_residuals_ordered)),
            "test_mean_rating_logit_weight": float(np.mean(rating_logit_weights_ordered)),
        ...
        print(f"[window {wi+1}/{len(windows)}] best_ep={info['best_epoch']:2d} "
              ...
              f"rΔ={np.mean(rating_logit_adjustments_ordered):.3f} rw={np.mean(rating_logit_weights_ordered):.3f} "
        ...
        pred_df["rating_logit_adjustment"] = rating_logit_adjustments_ordered
        pred_df["rating_feature_residual"] = rating_feature_residuals_ordered
        pred_df["rating_logit_weight"] = rating_logit_weights_ordered
    ...
    rating_logit_adjustments = predictions_df["rating_logit_adjustment"].to_numpy(dtype=float)
    rating_feature_residuals = predictions_df["rating_feature_residual"].to_numpy(dtype=float)
    rating_logit_weights = predictions_df["rating_logit_weight"].to_numpy(dtype=float)
    ...
    overall = {
        ...
        "mean_rating_logit_adjustment": float(rating_logit_adjustments.mean()),
        "mean_rating_feature_residual": float(rating_feature_residuals.mean()),
        "mean_rating_logit_weight": float(rating_logit_weights.mean()),
