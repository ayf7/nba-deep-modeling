#!/usr/bin/env python3
"""Train CME-Ratings-v1 (hybrid Sinkhorn + K-channel + involvement-supervised).
...
from cme_ratings_v1_common import (  # noqa: E402
    ...
    GameDatasetRatingsV1, build_records_ratings_v1, build_team_vocab,
    build_vocab_from_records_v42, collate_ratings_v1,
...
from cme_ratings_v1_model import CmeRatingsV1, CmeRatingsV1Config, total_loss  # noqa: E402


DEFAULT_OUTPUT_ROOT = REPO_ROOT / "models_cme_ratings_v1" / "artifacts"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--rating-hidden", type=int, default=32,
                   help="Hidden size for the dynamic-rating feature residual head.")
    p.add_argument("--rating-dropout", type=float, default=0.05,
                   help="Dropout for the dynamic-rating feature residual head.")
    p.add_argument("--max-rating-feature-residual", type=float, default=0.20,
                   help="Absolute tanh-bounded extra logit residual from rating features.")
    p.add_argument("--rating-margin-scale", type=float, default=12.0,
                   help="Convert the pregame rating margin prior from points to logits.")
    p.add_argument("--init-rating-logit-weight", type=float, default=0.10,
                   help="Initial additive weight on rating_margin_prior / rating_margin_scale.")
    ...
    p.add_argument("--rating-feature-reg-w", type=float, default=0.01,
                   help="L2 penalty on the bounded game-specific rating residual.")
    p.add_argument("--rating-weight-reg-w", type=float, default=0.0005,
                   help="Small L2 penalty on the scalar rating-margin blend weight.")
    # optim
...
def run_epoch(
    model: CmeRatingsV1, loader: DataLoader, *, device: str,
    ...
    sum_L_rating_feature_reg = 0.0
    sum_L_rating_weight_reg = 0.0
    ...
    sum_rating_logit_adjustment = 0.0
    sum_rating_feature_residual = 0.0
    sum_rating_logit_weight = 0.0
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(is_train):
            ...
            loss, diag = total_loss(
                ...
                rating_feature_reg_w=rating_feature_reg_w,
                rating_weight_reg_w=rating_weight_reg_w,
            )
        ...
        sum_L_rating_feature_reg += diag["L_rating_feature_reg"].item() * bs
        sum_L_rating_weight_reg += diag["L_rating_weight_reg"].item() * bs
        ...
        sum_rating_logit_adjustment += out["rating_logit_adjustment"].mean().item() * bs
        sum_rating_feature_residual += out["rating_feature_residual"].mean().item() * bs
        sum_rating_logit_weight += out["rating_logit_weight"].detach().item() * bs
        correct += correct_bs
    ...
    return {
        ...
        "L_rating_feature_reg": sum_L_rating_feature_reg / total_n,
        "L_rating_weight_reg": sum_L_rating_weight_reg / total_n,
        ...
        "mean_rating_logit_adjustment": sum_rating_logit_adjustment / total_n,
        "mean_rating_feature_residual": sum_rating_feature_residual / total_n,
        "rating_logit_weight": sum_rating_logit_weight / total_n,
        "alpha_off_corr": (
...
def main() -> None:
    ...
    train_recs = build_records_ratings_v1(
    ...
    val_recs = build_records_ratings_v1(
        val_df, matchup_rows=val_matchup, player_game_stats=val_pl,
        rating_lookup=rating_priors, rating_feature_stats=rating_feature_stats, **common,
    )
    test_recs = build_records_ratings_v1(
    ...
    train_loader = DataLoader(GameDatasetRatingsV1(train_recs), batch_size=args.batch_size,
                              shuffle=True, collate_fn=collate_ratings_v1)
    val_loader = DataLoader(GameDatasetRatingsV1(val_recs), batch_size=args.batch_size,
                            shuffle=False, collate_fn=collate_ratings_v1)
    test_loader = DataLoader(GameDatasetRatingsV1(test_recs), batch_size=args.batch_size,
                             shuffle=False, collate_fn=collate_ratings_v1)
    ...
    cfg = CmeRatingsV1Config(
        ...
        rating_hidden=args.rating_hidden,
        rating_dropout=args.rating_dropout,
        max_rating_feature_residual=args.max_rating_feature_residual,
        rating_margin_scale=args.rating_margin_scale,
        init_rating_logit_weight=args.init_rating_logit_weight,
    )
    model = CmeRatingsV1(cfg).to(args.device)
    ...
    for name, param in model.named_parameters():
        ...
        if (name in calibration_param_names
                ...
                or name == "rating_logit_weight"
                or name.startswith("rating_prior_head.")):
    ...
    print(f"[loss] team_w={args.team_w} player_w={args.player_w} "
          ...
          f"rating_feature_reg_w={args.rating_feature_reg_w} "
          f"rating_weight_reg_w={args.rating_weight_reg_w}")
        ...
        torch.save(
            {"model_state": model.state_dict(), "cfg": asdict(cfg),
             ...
             "loss_level_weights": {
                 ...
                 "rating_feature_reg": args.rating_feature_reg_w,
                 "rating_weight_reg": args.rating_weight_reg_w,
             },
             "epoch": epoch},
            path,
        )
    ...
    def epoch_kwargs() -> dict:
        return dict(
            ...
            rating_feature_reg_w=args.rating_feature_reg_w,
            rating_weight_reg_w=args.rating_weight_reg_w,
        )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        row = {
            ...
            "train_mean_rating_logit_adjustment": tr["mean_rating_logit_adjustment"], "val_mean_rating_logit_adjustment": va["mean_rating_logit_adjustment"],
            "train_rating_logit_weight": tr["rating_logit_weight"], "val_rating_logit_weight": va["rating_logit_weight"],
        }
        history.append(row)
        extra = ""
        ...
        print(
            ...
            f"rM={tr['mean_rating_margin_prior']:.2f} rΔ={tr['mean_rating_logit_adjustment']:.3f} "
            f"rw={tr['rating_logit_weight']:.3f} | "
            ...
            f"va_rM={va['mean_rating_margin_prior']:.2f} va_rΔ={va['mean_rating_logit_adjustment']:.3f} "
            f"va_rw={va['rating_logit_weight']:.3f}{extra} "
    ...
    print(
        ...
        f"rM={te_best['mean_rating_margin_prior']:.2f} rΔ={te_best['mean_rating_logit_adjustment']:.3f} "
        f"rw={te_best['rating_logit_weight']:.3f}"
    )
