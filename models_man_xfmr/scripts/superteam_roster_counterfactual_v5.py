#!/usr/bin/env python3
"""V5/ManXfmr roster counterfactual: add a superteam to Lakers March 2026.

Loads a per-window backtest checkpoint and evaluates two OOS scenarios on
Lakers games in March 2026:
...
Mirrors the v1/v2 counterfactual scripts; v5's ManXfmr does not predict
points, so only win probabilities are reported.
"""
...
from man_xfmr_model import ManXfmr, XfmrConfig  # noqa: E402


DEFAULT_ART = REPO_ROOT / "models_man_xfmr" / "artifacts"
DEFAULT_CHECKPOINT = DEFAULT_ART / "backtest_v5_ckpt" / "windows" / "2026-03-01" / "best.pt"
DEFAULT_PLAYERS = (
...
def collect_predictions(
    ...
    *,
    ...
    loader = DataLoader(XfmrGameDataset(records), batch_size=batch_size,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--run-name", type=str, default="superteam_roster_cf_2026_03_v5backtest")
...
def modify_lakers_record(record, *, lakers_home: bool, additions: list[dict], mode: str):
    if lakers_home:
...
def plot_results(df: pd.DataFrame, out_path: Path, scenario_label: str) -> None:
    ...
    ax1.set_ylim(0, 1)
    ax1.set_title(f"V5/ManXfmr Lakers March 2026 roster counterfactual: add {scenario_label}")
    ...
    summary = [
        f"added mean={df['delta_superteam_added'].mean():+.3f}",
        f"replace mean={df['delta_superteam_replace_last4'].mean():+.3f}",
    ]
...
def main() -> None:
    ...
    baseline_records = build_records(test_df, **common_kwargs)
