"""CME-v4 data layer (hybrid: v2 channels + per-player involvement shares).

v4 builds on v2's K_PAIR=9 / K_PLAYER=6 / K_BOX=14 supervision and adds
...
# Low-dimensional season-phase context for v4.2 calibration.  The rolling
# backtests showed calibration drift by monthly window even after the v4.1
# global affine correction.  These features are deterministic from the game
# date, available pregame, and generalize across seasons.
SEASON_PHASE_DIM = 3
SEASON_PHASE_DAYS = 240.0


def season_phase_features(game_date: pd.Timestamp) -> tuple[float, float, float]:
    """Return centered-linear + sin/cos season progress features.

    NBA seasons span calendar years.  We anchor the phase at October 1 of the
    season year, clip at 240 days (regular-season + modest postseason buffer),
    and provide a smooth 3-vector for the calibration head:

        [2*phase - 1, sin(2π phase), cos(2π phase)]

    The features are deterministic and do not leak future outcomes.
    """
    gd = pd.Timestamp(game_date).normalize()
    season_year = gd.year if gd.month >= 7 else gd.year - 1
    season_start = pd.Timestamp(year=season_year, month=10, day=1)
    days = float((gd - season_start).days)
    phase = min(max(days / SEASON_PHASE_DAYS, 0.0), 1.0)
    angle = 2.0 * math.pi * phase
    return (2.0 * phase - 1.0, math.sin(angle), math.cos(angle))
...
class GameRecordV42NoTemporal:
    ...
    # v4 additions: per-player actual involvement shares (sum to 1 on each side)
    ...
    # v4.2: deterministic pregame season-phase features used only by the
    # calibration pathway, not the structured scoring backbone.
    season_phase: tuple[float, float, float]
...
def build_records_v42(
    ...
    *,
...
) -> list[GameRecordV42NoTemporal]:
    """Build per-game v4 records. Identical to v2 but additionally computes
    ...
    out: list[GameRecordV42NoTemporal] = []
    for row_i, game in enumerate(games.itertuples(index=False)):
        ...
        # --- v4 addition: per-player involvement shares ---
        ...
        out.append(GameRecordV42NoTemporal(
            game_id=gid,
            game_date=gd,
            ...
            season_phase=season_phase_features(gd),
            home_dec_odds=h_dec,
            away_dec_odds=a_dec,
            has_odds=has_odds,
        ))
...
def build_vocab_from_records_v42(
...
class GameDatasetV42NoTemporal(Dataset):
    def __init__(self, records: list[GameRecordV42NoTemporal]) -> None:
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        return {
            ...
            "season_phase": torch.tensor(r.season_phase, dtype=torch.float32),
            ...
            "sup_pl_y": sup_pl_y,
            # v4: per-player involvement-share targets (sum-to-1 on each side)
...
def collate_v42_no_temporal(batch: list[dict]) -> dict:
    """Pad rosters; concat per-game flattened sup_* tensors. Adds v4 fields."""
    B = len(batch)
    ...
    # v4 actual-share targets (padded with 0; mask handled by sum check at loss time)
    ...
    out = {
        ...
        "season_phase": torch.stack([b["season_phase"] for b in batch]),
...
__all__ = [
    ...
    "MIN_EXPOSURE_SECONDS", "SEASON_PHASE_DIM", "SEASON_PHASE_DAYS",
    "season_phase_features",
    # reused utilities
    ...
    # v4 additions
    "GameRecordV42NoTemporal", "build_records_v42", "build_vocab_from_records_v42",
    "GameDatasetV42NoTemporal", "collate_v42_no_temporal", "load_pair_exposure_seconds",
]
