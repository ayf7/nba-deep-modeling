"""CME-v4.2-no-temporal: hybrid world model (Sinkhorn budget + K-channel play-style).
...
Forward pipeline per game:
 ...
 10. Assemble per-player BOX (14-d) -> team BOX -> margin -> calibrated win_logit with the temporal season branch ablated.
...
class CmeV42NoTemporalConfig:
    ...
    # v4.2 calibrated-margin head.  v4.1 fixed the global home-bias miss,
    # but its calibration still drifted by rolling window and its context
    # residual made the extreme probability tail a little too sharp.  v4.2
...
class CmeV42NoTemporal(nn.Module):
    def __init__(self, cfg: CmeV42NoTemporalConfig) -> None:
        super().__init__()
        self.cfg = cfg
        d = cfg.d
        ...
        # -- margin -> win logit, with v4.2-no-temporal affine calibration --
        ...
        # global bias remained too close to zero.  v4.2 elevates the low-risk
        ...
        # Temporal/season calibration is intentionally removed in this ablation.
        # We keep neutral diagnostic outputs later in forward() so prediction CSVs
        # remain easy to compare against CME-v4.2.
    ...
    def forward(self, batch: dict) -> dict:
        ...
        # No temporal branch in this ablation: the affine probability path feeds
        # the tail-gated residual directly.  These neutral aliases preserve the
        # v4.2 prediction/output schema for apples-to-apples analysis.
        ...
        win_logit_temporal = win_logit_affine
