#!/usr/bin/env python3
"""Expanding-window monthly backtest for CME-v4.2.
...
Defaults match `train_cme_v4_2_no_temporal.py`'s nine-level loss
...
V42_SCRIPTS = Path(__file__).resolve().parent
sys.path.insert(0, str(V42_SCRIPTS))

from cme_v4_2_no_temporal_common import (  # noqa: E402
    ...
    GameDatasetV42NoTemporal, build_records_v42, build_team_vocab,
    build_vocab_from_records_v42, collate_v42_no_temporal,
    fit_tabular_stats,
...
from cme_v4_2_no_temporal_model import CmeV42NoTemporal, CmeV42NoTemporalConfig  # noqa: E402
from train_cme_v4_2_no_temporal import (  # noqa: E402
    default_box_weights, default_pair_weights, run_epoch,
)


DEFAULT_OUTPUT_ROOT = REPO_ROOT / "models_cme_v4_2_no_temporal" / "artifacts"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--max-calibration-residual", type=float, default=0.30,
                   help="Absolute tanh-bounded context residual after affine calibration.")
    p.add_argument("--season-calibration-hidden", type=int, default=16,
                   help="Compatibility no-op in the no-temporal ablation; retained for argument parity.")
    p.add_argument("--max-season-logit-adjustment", type=float, default=0.25,
                   help="Compatibility no-op in the no-temporal ablation; season adjustment is fixed at zero.")
    p.add_argument("--tail-gate-center", type=float, default=1.25,
                   help="Absolute affine logit where the residual gate is 0.5.")
    ...
    # nine-level loss weights (match train_cme_v4_2_no_temporal defaults)
...
def predict_window(
    model: CmeV42NoTemporal, records: list, batch_size: int, device: str,
) -> tuple[np.ndarray, ...]:
    model.eval()
    loader = DataLoader(GameDatasetV42NoTemporal(records), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_v42_no_temporal)
...
def _build_optimizer(model: CmeV42NoTemporal, args) -> torch.optim.Optimizer:
...
def _per_task_grad_norms(
    ...
    from cme_v4_2_no_temporal_model import (
...
def _inspect_player_predictions(
    ...
    loader = DataLoader(GameDatasetV42NoTemporal(sample), batch_size=len(sample),
                        shuffle=False, collate_fn=collate_v42_no_temporal)
    pts_i = BOX_INDEX["pts"]
    fgm_i = BOX_INDEX["fgm"]
    fga_i = BOX_INDEX["fga"]
...
def _track_player_predictions(
    ...
    loader = DataLoader(GameDatasetV42NoTemporal(test_recs), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_v42_no_temporal)
    model.eval()
...
def train_one_window(
...
) -> tuple[CmeV42NoTemporal, dict, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    train_recs = build_records_v42(
        train_fit_df, matchup_rows=train_matchup, player_game_stats=train_pl, **common,
    )
    val_recs = build_records_v42(
        val_df, matchup_rows=val_matchup, player_game_stats=val_pl, **common,
    )
    test_recs = build_records_v42(
        test_df, matchup_rows=test_matchup, player_game_stats=test_pl, **common,
    )

    train_loader = DataLoader(GameDatasetV42NoTemporal(train_recs), batch_size=args.batch_size,
                              shuffle=True, collate_fn=collate_v42_no_temporal)
    val_loader = DataLoader(GameDatasetV42NoTemporal(val_recs), batch_size=args.batch_size,
                            shuffle=False, collate_fn=collate_v42_no_temporal)

    import cme_v4_2_no_temporal_common as _cv4c
    tabular_dim = 0 if args.no_tabular else len(_cv4c.TABULAR_FEATURE_COLUMNS)
    cfg = CmeV42NoTemporalConfig(
    ...
    model = CmeV42NoTemporal(cfg).to(args.device)
