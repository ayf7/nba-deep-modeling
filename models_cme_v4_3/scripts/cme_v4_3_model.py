"""CME-v4.3: hybrid world model (Sinkhorn budget + K-channel play-style).
...
Nine-level loss:
    L = w_inv * L_inv_mse           (alpha vs actual exposure shares)
      ...
      + w_matchup * L_matchup_evidence_reg (bounded lineup/matchup evidence residual)
...
class CmeV43Config:
    ...
    # v4.3 calibrated-margin head.  v4.1 fixed the global home-bias miss,
    # but its calibration still drifted by rolling window and its context
    # residual made the extreme probability tail a little too sharp.  v4.3
    # retains the affine margin head and adds:
    #   (a) a small season-phase calibration adjustment, and
    #   (b) a tail gate that suppresses residual overreaction on extreme logits.
    #
    # final_logit = slope * (margin/scale) + home_bias
    #             + season_delta(phase)
    #             + tail_gate(logit) * bounded_residual(ctx)
    #             + tail_gate(logit) * bounded_matchup_evidence(player pools)
    #
    # v4.3 is a targeted ablation: it keeps the proven v4.2 stack and tests
    # only whether pooled lineup/matchup state can add out-of-sample signal.
    ...
    matchup_evidence_hidden: int = 64
    max_matchup_evidence_residual: float = 0.20
...
class CmeV43(nn.Module):
    def __init__(self, cfg: CmeV43Config) -> None:
        super().__init__()
        self.cfg = cfg
        d = cfg.d
        ...
        # -- margin -> win logit, with v4.3 affine calibration --
        ...
        # global bias remained too close to zero.  v4.3 elevates the low-risk
        ...
        # v4.3 ablation: a tightly bounded residual that can see pooled lineup
        # and cross-attended matchup states.  It is zero-initialized so the
        # model starts exactly at the v4.2 probability path.
    ...
    def forward(self, batch: dict) -> dict:
        ...
        if season_phase is None:
            # Defensive fallback for ad-hoc synthetic callers; real v4.3
        ...
        # Pooled lineup/matchup evidence residual.  This branch intentionally
        # does not alter the world-model box-score path; it only contributes a
        # small, regularized probability correction on top of the v4.2 logit.
        ...
        win_logit = win_logit_temporal + win_residual + matchup_evidence_residual
...
def matchup_evidence_l2_loss(out: dict) -> torch.Tensor:
    """Keep the pooled lineup/matchup evidence residual close to zero."""
...
def _smoke() -> None:
    ...
    print(f"\nloss = {loss.item():.4f}")
    for k in ("L_team", "L_player", "L_pair", "L_inv", "L_win", "L_margin_nll", "L_calibration_reg", "L_matchup_evidence_reg", "L_season_calibration_reg", "L_calibration_slope_reg"):
