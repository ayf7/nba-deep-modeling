#!/usr/bin/env python3
"""Expanding-window monthly backtest for CME-v4.3.
...
Defaults match `train_cme_v4_3.py`'s nine-level loss
...
V43_SCRIPTS = Path(__file__).resolve().parent
sys.path.insert(0, str(V43_SCRIPTS))

from cme_v4_3_common import (  # noqa: E402
    ...
    GameDatasetV43, build_records_v43, build_team_vocab,
    build_vocab_from_records_v43, collate_v43,
    fit_tabular_stats,
...
from cme_v4_3_model import CmeV43, CmeV43Config  # noqa: E402
from train_cme_v4_3 import (  # noqa: E402
    default_box_weights, default_pair_weights, run_epoch,
)


DEFAULT_OUTPUT_ROOT = REPO_ROOT / "models_cme_v4_3" / "artifacts"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--calibration-hidden", type=int, default=64,
                   help="Hidden size for the v4.3 context-residual head.")
    p.add_argument("--calibration-dropout", type=float, default=0.05,
                   help="Dropout inside the v4.3 context-residual head.")
    ...
    p.add_argument("--season-calibration-hidden", type=int, default=16,
                   help="Hidden size for the v4.3 season-phase calibration head.")
    ...
    p.add_argument("--matchup-evidence-hidden", type=int, default=64,
                   help="Hidden size for the tightly bounded pooled-lineup matchup evidence head.")
    p.add_argument("--max-matchup-evidence-residual", type=float, default=0.20,
                   help="Absolute tanh-bounded lineup/matchup evidence residual before final logits.")
    ...
    # ten-term loss weights (match train_cme_v4_3 defaults)
    ...
    p.add_argument("--calibration-reg-w", type=float, default=0.075,
                   help="L2 penalty weight on v4.3's tail-gated context residual.")
    p.add_argument("--matchup-evidence-reg-w", type=float, default=0.10,
                   help="L2 penalty weight on the pooled-lineup matchup evidence residual.")
...
def predict_window(
    model: CmeV43, records: list, batch_size: int, device: str,
) -> tuple[np.ndarray, ...]:
    model.eval()
    loader = DataLoader(GameDatasetV43(records), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_v43)
...
def _build_ckpt_payload(model, cfg, vocab, team_vocab, box_weights, pair_weights,
                        ...
                        calibration_reg_w, matchup_evidence_reg_w, season_calibration_reg_w, calibration_slope_reg_w, epoch):
    return {
...
def _build_optimizer(model: CmeV43, args) -> torch.optim.Optimizer:
...
def _inspect_player_predictions(
    ...
    loader = DataLoader(GameDatasetV43(sample), batch_size=len(sample),
                        shuffle=False, collate_fn=collate_v43)
    pts_i = BOX_INDEX["pts"]
    fgm_i = BOX_INDEX["fgm"]
    fga_i = BOX_INDEX["fga"]
...
def _track_player_predictions(
    ...
    loader = DataLoader(GameDatasetV43(test_recs), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_v43)
    model.eval()
...
def train_one_window(
...
) -> tuple[CmeV43, dict, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    vocab = build_vocab_from_records_v43(
    ...
    train_recs = build_records_v43(
        train_fit_df, matchup_rows=train_matchup, player_game_stats=train_pl, **common,
    )
    val_recs = build_records_v43(
        val_df, matchup_rows=val_matchup, player_game_stats=val_pl, **common,
    )
    test_recs = build_records_v43(
        test_df, matchup_rows=test_matchup, player_game_stats=test_pl, **common,
    )

    train_loader = DataLoader(GameDatasetV43(train_recs), batch_size=args.batch_size,
                              shuffle=True, collate_fn=collate_v43)
    val_loader = DataLoader(GameDatasetV43(val_recs), batch_size=args.batch_size,
                            shuffle=False, collate_fn=collate_v43)

    import cme_v4_3_common as _cv43c
    tabular_dim = 0 if args.no_tabular else len(_cv43c.TABULAR_FEATURE_COLUMNS)
    cfg = CmeV43Config(
    ...
    model = CmeV43(cfg).to(args.device)
    ...
    if ckpt_dir is not None and args.save_final:
        final_payload = _build_ckpt_payload(
            ...
            ckpt_margin_nll_w, ckpt_calibration_reg_w, ckpt_matchup_evidence_reg_w, ckpt_season_calibration_reg_w, ckpt_calibration_slope_reg_w, last_epoch,
        )
    else:
        final_payload = None
    ...
    if ckpt_dir is not None:
        ...
        torch.save(
            _build_ckpt_payload(
                model, cfg, vocab, team_vocab, ckpt_box_weights, ckpt_pair_weights,
                ckpt_team_w, ckpt_player_w, ckpt_pair_w, ckpt_inv_w, ckpt_win_w,
                ckpt_margin_nll_w, ckpt_calibration_reg_w, ckpt_matchup_evidence_reg_w, ckpt_season_calibration_reg_w, ckpt_calibration_slope_reg_w, best_epoch,
            ),
            ckpt_dir / "best.pt",
        )
    ...
    gids, probs, base_probs, affine_probs, temporal_probs, season_logit_adjustments, residual_tail_gates, win_residuals, matchup_evidence_residuals, home_logit_biases, win_logit_slopes, h_pts, a_pts, margin_mu = predict_window(
    ...
    return model, window_info, gids, probs, base_probs, affine_probs, temporal_probs, season_logit_adjustments, residual_tail_gates, win_residuals, matchup_evidence_residuals, home_logit_biases, win_logit_slopes, h_pts, a_pts, margin_mu
...
def main() -> None:
    ...
    if args.no_cyclic:
        ...
        _cv43c.TABULAR_FEATURE_COLUMNS = trimmed
    ...
    for wi, window_start in enumerate(windows):
        ...
        try:
            _model, info, gids, probs, base_probs, affine_probs, temporal_probs, season_logit_adjustments, residual_tail_gates, win_residuals, matchup_evidence_residuals, home_logit_biases, win_logit_slopes, h_pts, a_pts, margin_mu = train_one_window(
