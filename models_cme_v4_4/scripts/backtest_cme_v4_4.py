#!/usr/bin/env python3
"""Expanding-window monthly backtest for CME-v4.4.
...
Defaults match `train_cme_v4_4.py`'s nine-level loss
...
V44_SCRIPTS = Path(__file__).resolve().parent
sys.path.insert(0, str(V44_SCRIPTS))

from cme_v4_4_common import (  # noqa: E402
    BOX_INDEX, K_BOX, K_PAIR, K_PROCESS_FEATURES,
    ...
    GameDatasetV44, build_records_v44, build_team_vocab,
    build_vocab_from_records_v42, collate_v44,
    fit_process_feature_stats, fit_tabular_stats,
    load_game_odds, load_game_player_status, load_game_scores, load_games,
    load_matchup_rows_v2, load_player_game_stats, load_process_pregame_features,
    load_status_calibration, load_team_exposures,
)
from cme_v4_4_model import CmeV44, CmeV44Config  # noqa: E402
from train_cme_v4_4 import (  # noqa: E402
    default_box_weights, default_pair_weights, run_epoch,
)


DEFAULT_OUTPUT_ROOT = REPO_ROOT / "models_cme_v4_4" / "artifacts"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--no-process-features", action="store_true",
                   help="Disable rolling process-style pregame features for a direct v4.2-equivalent ablation.")
...
def predict_window(
    model: CmeV44, records: list, batch_size: int, device: str,
) -> tuple[np.ndarray, ...]:
    model.eval()
    loader = DataLoader(GameDatasetV44(records), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_v44)
...
def _build_ckpt_payload(model, cfg, vocab, team_vocab, box_weights, pair_weights,
                        ...
                        calibration_reg_w, season_calibration_reg_w, calibration_slope_reg_w, epoch,
                        process_feature_stats=None):
    return {
        ...
        "process_feature_columns": list(PROCESS_PREGAME_FEATURE_COLUMNS),
        "process_feature_medians": (process_feature_stats.medians.tolist() if process_feature_stats is not None else None),
        "process_feature_means": (process_feature_stats.means.tolist() if process_feature_stats is not None else None),
        "process_feature_stds": (process_feature_stats.stds.tolist() if process_feature_stats is not None else None),
        "loss_level_weights": {
...
def _build_optimizer(model: CmeV44, args) -> torch.optim.Optimizer:
...
def _inspect_player_predictions(
    ...
    loader = DataLoader(GameDatasetV44(sample), batch_size=len(sample),
                        shuffle=False, collate_fn=collate_v44)
    pts_i = BOX_INDEX["pts"]
    fgm_i = BOX_INDEX["fgm"]
    fga_i = BOX_INDEX["fga"]
...
def _track_player_predictions(
    ...
    loader = DataLoader(GameDatasetV44(test_recs), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_v44)
    model.eval()
...
def train_one_window(
    ...
    histories, scores, statuses, calibration, game_odds, process_features,
    matchup_db: Path, core_db: Path,
    ckpt_dir: Path | None = None,
) -> tuple[CmeV44, dict, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    train_recs = build_records_v44(
        train_fit_df, matchup_rows=train_matchup, player_game_stats=train_pl, **common,
    )
    val_recs = build_records_v44(
        val_df, matchup_rows=val_matchup, player_game_stats=val_pl, **common,
    )
    test_recs = build_records_v44(
        test_df, matchup_rows=test_matchup, player_game_stats=test_pl, **common,
    )

    train_loader = DataLoader(GameDatasetV44(train_recs, include_base_tabular=not args.no_tabular), batch_size=args.batch_size,
                              shuffle=True, collate_fn=collate_v44)
    val_loader = DataLoader(GameDatasetV44(val_recs, include_base_tabular=not args.no_tabular), batch_size=args.batch_size,
                            shuffle=False, collate_fn=collate_v44)

    import cme_v4_4_common as _cv4c
    base_tabular_dim = 0 if args.no_tabular else len(_cv4c.TABULAR_FEATURE_COLUMNS)
    process_tabular_dim = 0 if args.no_process_features else K_PROCESS_FEATURES
    tabular_dim = base_tabular_dim + process_tabular_dim
    cfg = CmeV44Config(
    ...
    model = CmeV44(cfg).to(args.device)
    ...
    if ckpt_dir is not None and args.save_final:
        final_payload = _build_ckpt_payload(
            ...
            ckpt_margin_nll_w, ckpt_calibration_reg_w, ckpt_season_calibration_reg_w, ckpt_calibration_slope_reg_w, last_epoch,
            process_feature_stats=process_feature_stats,
        )
    else:
        final_payload = None
    ...
    if ckpt_dir is not None:
        ...
        torch.save(
            _build_ckpt_payload(
                model, cfg, vocab, team_vocab, ckpt_box_weights, ckpt_pair_weights,
                ckpt_team_w, ckpt_player_w, ckpt_pair_w, ckpt_inv_w, ckpt_win_w,
                ckpt_margin_nll_w, ckpt_calibration_reg_w, ckpt_season_calibration_reg_w, ckpt_calibration_slope_reg_w, best_epoch,
                process_feature_stats=process_feature_stats,
            ),
            ckpt_dir / "best.pt",
        )
    ...
    gids, probs, base_probs, affine_probs, temporal_probs, season_logit_adjustments, residual_tail_gates, win_residuals, home_logit_biases, win_logit_slopes, h_pts, a_pts, margin_mu = predict_window(
    ...
    window_info = {
        ...
        "process_tabular_dim": process_tabular_dim,
        "best_epoch": best_epoch,
        "best_val_bce": best_val,
        "epochs_run": len(history),
        "history": history,
    }
    return model, window_info, gids, probs, base_probs, affine_probs, temporal_probs, season_logit_adjustments, residual_tail_gates, win_residuals, home_logit_biases, win_logit_slopes, h_pts, a_pts, margin_mu
...
def main() -> None:
    ...
    print(f"[load] process_features={len(process_features)}")
    ...
    for wi, window_start in enumerate(windows):
        ...
        try:
            _model, info, gids, probs, base_probs, affine_probs, temporal_probs, season_logit_adjustments, residual_tail_gates, win_residuals, home_logit_biases, win_logit_slopes, h_pts, a_pts, margin_mu = train_one_window(
                args, train_fit, val, test_block, histories, scores, statuses,
                calibration, game_odds, process_features, args.matchup_db, args.core_db,
                ckpt_dir=ckpt_dir,
            )
        ...
        print(f"[window {wi+1}/{len(windows)}] best_ep={info['best_epoch']:2d} "
              ...
              f"δ={np.mean(win_residuals_ordered):.3f} b={np.mean(home_logit_biases_ordered):.3f} "
