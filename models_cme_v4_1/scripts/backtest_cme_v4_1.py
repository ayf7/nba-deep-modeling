#!/usr/bin/env python3
"""Expanding-window monthly backtest for CME-v4.1.
...
Defaults match `train_cme_v4_1.py`'s seven-level loss
...
V41_SCRIPTS = Path(__file__).resolve().parent
sys.path.insert(0, str(V41_SCRIPTS))

from cme_v4_1_common import (  # noqa: E402
    ...
    GameDatasetV41, build_records_v41, build_team_vocab,
    build_vocab_from_records_v41, collate_v41,
    fit_tabular_stats,
...
from cme_v4_1_model import CmeV41, CmeV41Config  # noqa: E402
from train_cme_v4_1 import (  # noqa: E402
    default_box_weights, default_pair_weights, run_epoch,
)


DEFAULT_OUTPUT_ROOT = REPO_ROOT / "models_cme_v4_1" / "artifacts"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--calibration-hidden", type=int, default=64,
                   help="Hidden size for the v4.1 context-residual head.")
    p.add_argument("--calibration-dropout", type=float, default=0.05,
                   help="Dropout inside the v4.1 context-residual head.")
    p.add_argument("--max-calibration-residual", type=float, default=0.35,
                   help="Absolute tanh-bounded context residual after affine margin calibration.")
    ...
    # seven-level loss weights (match train_cme_v4_1 defaults)
    ...
    p.add_argument("--calibration-reg-w", type=float, default=0.05,
                   help="L2 penalty weight on v4.1's bounded context residual.")
...
def predict_window(
    model: CmeV41, records: list, batch_size: int, device: str,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
    loader = DataLoader(GameDatasetV41(records), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_v41)
...
def _build_ckpt_payload(model, cfg, vocab, team_vocab, box_weights, pair_weights,
                        ...
                        calibration_reg_w, calibration_slope_reg_w, epoch):
    return {
...
def _build_optimizer(model: CmeV41, args) -> torch.optim.Optimizer:
    ...
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        if name in calibration_param_names or name.startswith("win_calibration_head."):
...
def _inspect_player_predictions(
    ...
    loader = DataLoader(GameDatasetV41(sample), batch_size=len(sample),
                        shuffle=False, collate_fn=collate_v41)
    pts_i = BOX_INDEX["pts"]
    fgm_i = BOX_INDEX["fgm"]
    fga_i = BOX_INDEX["fga"]
...
def _track_player_predictions(
    ...
    loader = DataLoader(GameDatasetV41(test_recs), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_v41)
    model.eval()
...
def train_one_window(
...
) -> tuple[CmeV41, dict, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    vocab = build_vocab_from_records_v41(
    ...
    train_recs = build_records_v41(
        train_fit_df, matchup_rows=train_matchup, player_game_stats=train_pl, **common,
    )
    val_recs = build_records_v41(
        val_df, matchup_rows=val_matchup, player_game_stats=val_pl, **common,
    )
    test_recs = build_records_v41(
        test_df, matchup_rows=test_matchup, player_game_stats=test_pl, **common,
    )

    train_loader = DataLoader(GameDatasetV41(train_recs), batch_size=args.batch_size,
                              shuffle=True, collate_fn=collate_v41)
    val_loader = DataLoader(GameDatasetV41(val_recs), batch_size=args.batch_size,
                            shuffle=False, collate_fn=collate_v41)

    import cme_v4_1_common as _cv4c
    ...
    model = CmeV41(cfg).to(args.device)
    ...
    if ckpt_dir is not None and args.save_final:
        final_payload = _build_ckpt_payload(
            ...
            ckpt_margin_nll_w, ckpt_calibration_reg_w, ckpt_calibration_slope_reg_w, last_epoch,
        )
    else:
        final_payload = None
    ...
    if ckpt_dir is not None:
        ...
        torch.save(
            _build_ckpt_payload(
                model, cfg, vocab, team_vocab, ckpt_box_weights, ckpt_pair_weights,
                ckpt_team_w, ckpt_player_w, ckpt_pair_w, ckpt_inv_w, ckpt_win_w,
                ckpt_margin_nll_w, ckpt_calibration_reg_w, ckpt_calibration_slope_reg_w, best_epoch,
            ),
            ckpt_dir / "best.pt",
        )
    ...
    gids, probs, base_probs, affine_probs, win_residuals, home_logit_biases, win_logit_slopes, h_pts, a_pts, margin_mu = predict_window(
    ...
    return model, window_info, gids, probs, base_probs, affine_probs, win_residuals, home_logit_biases, win_logit_slopes, h_pts, a_pts, margin_mu
...
def main() -> None:
    ...
    for wi, window_start in enumerate(windows):
        ...
        try:
            _model, info, gids, probs, base_probs, affine_probs, win_residuals, home_logit_biases, win_logit_slopes, h_pts, a_pts, margin_mu = train_one_window(
        ...
        print(f"[window {wi+1}/{len(windows)}] best_ep={info['best_epoch']:2d} "
              ...
              f"aff={np.mean(affine_probs_ordered):.3f} δ={np.mean(win_residuals_ordered):.3f} "
