"""CME-v4.1: hybrid world model (Sinkhorn budget + K-channel play-style).
...
class CmeV41Config:
    ...
    # v4.1 calibrated-margin head.  The v4 backtest moved aggregate home
    # probabilities in the right direction, but the flexible residual carried
    # most of the correction while the global intercept stayed too small.
    # v4.1 makes the calibration path more explicit:
    #   win_logit = slope * (margin/scale) + home_bias + bounded_residual(ctx)
    calibration_hidden: int = 64
    calibration_dropout: float = 0.05
    max_calibration_residual: float = 0.35
...
class CmeV41(nn.Module):
    def __init__(self, cfg: CmeV41Config) -> None:
        super().__init__()
        self.cfg = cfg
        d = cfg.d
        ...
        # -- margin -> win logit, with v4.1 affine calibration --
        ...
        # global bias remained too close to zero.  v4.1 elevates the low-risk
    ...
    def forward(self, batch: dict) -> dict:
        ...
        calibration_features = torch.cat([
            ...
            home_ctx - away_ctx,
            home_ctx * away_ctx,
            win_logit_affine.unsqueeze(-1),
        ], dim=-1)
        raw_win_residual = self.win_calibration_head(calibration_features).squeeze(-1)
        if self.cfg.max_calibration_residual > 0.0:
            win_residual = self.cfg.max_calibration_residual * torch.tanh(raw_win_residual)
        else:
            win_residual = torch.zeros_like(win_logit_affine)
        win_logit = win_logit_affine + win_residual
...
def calibration_residual_l2_loss(out: dict) -> torch.Tensor:
    """Keep the context-specific residual small so affine calibration leads.

    v4's residual branch helped accuracy, but it absorbed most of the global
    home-side correction.  v4.1 keeps the residual available for genuinely
    context-specific effects while discouraging it from replacing the explicit
    affine calibration path.
    """
...
def _smoke() -> None:
    ...
    print(f"\nloss = {loss.item():.4f}")
    for k in ("L_team", "L_player", "L_pair", "L_inv", "L_win", "L_margin_nll", "L_calibration_reg", "L_calibration_slope_reg"):
