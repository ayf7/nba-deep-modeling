#!/usr/bin/env python3
"""Expanding-window monthly backtest for CME-Ratings-v3.
...
single CSV with the same schema as the baseline / v3 backtests
...
from cme_ratings_v3_common import (  # noqa: E402
    BOX_INDEX, K_BOX, K_PAIR, K_RATING_FEATURES, K_RATING_TRACKS,
...
from cme_ratings_v3_model import CmeRatingsV3, CmeRatingsV3Config  # noqa: E402
from train_cme_ratings_v3 import (  # noqa: E402
...
def predict_window(
    model: CmeRatingsV3, records: list, batch_size: int, device: str,
) -> tuple[np.ndarray, ...]:
    model.eval()
    loader = DataLoader(GameDatasetRatingsV3(records), batch_size=batch_size,
...
def _build_optimizer(model: CmeRatingsV3, args) -> torch.optim.Optimizer:
...
def _inspect_player_predictions(
    ...
    loader = DataLoader(GameDatasetRatingsV3(sample), batch_size=len(sample),
                        shuffle=False, collate_fn=collate_ratings_v3)
    pts_i = BOX_INDEX["pts"]
    fgm_i = BOX_INDEX["fgm"]
    fga_i = BOX_INDEX["fga"]
...
def _track_player_predictions(
    ...
    loader = DataLoader(GameDatasetRatingsV3(test_recs), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_ratings_v3)
    model.eval()
