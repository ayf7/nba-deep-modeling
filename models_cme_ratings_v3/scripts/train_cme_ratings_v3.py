#!/usr/bin/env python3
"""Train CME-Ratings-v3 (hybrid Sinkhorn + K-channel + involvement-supervised).
...
from cme_ratings_v3_common import (  # noqa: E402
    BOX_INDEX, BOX_TARGETS, K_BOX, K_PAIR, K_RATING_FEATURES, K_RATING_TRACKS, PAIR_TARGETS,
    ...
    GameDatasetRatingsV3, build_records_ratings_v3, build_team_vocab,
    build_vocab_from_records_v42, collate_ratings_v3,
    fit_player_form_stats, fit_rating_feature_stats, fit_tabular_stats,
...
from cme_ratings_v3_model import CmeRatingsV3, CmeRatingsV3Config, total_loss  # noqa: E402


DEFAULT_OUTPUT_ROOT = REPO_ROOT / "models_cme_ratings_v3" / "artifacts"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--ratings-db", type=Path, default=DEFAULT_RATINGS_DB,
                   help="Dynamic team ratings artifact from data/scripts/build_team_strength_ratings_v3.py.")
    ...
    p.add_argument("--init-rating-logit-bias", type=float, default=0.04,
                   help="Warm start for the ratings-ensemble calibration intercept.")
    p.add_argument("--init-rating-track-slopes", type=float, nargs=3, default=(1.45, 1.50, 1.55),
                   help="Warm starts for slow/medium/fast ratings-prior calibration slopes.")
    p.add_argument("--rating-mix-hidden", type=int, default=32,
                   help="Hidden size for the ratings-only slow/medium/fast mixture gate.")
    p.add_argument("--rating-mix-dropout", type=float, default=0.05,
                   help="Dropout inside the ratings-only mixture gate.")
    p.add_argument("--max-rating-mix-logit-delta", type=float, default=0.75,
                   help="Tanh bound for data-dependent mixture-logit deviations from the initialized base weights.")
...
def run_epoch(
    model: CmeRatingsV3, loader: DataLoader, *, device: str,
...
def main() -> None:
    ...
    train_recs = build_records_ratings_v3(
    ...
    val_recs = build_records_ratings_v3(
        val_df, matchup_rows=val_matchup, player_game_stats=val_pl,
        rating_lookup=rating_priors, rating_feature_stats=rating_feature_stats, **common,
    )
    test_recs = build_records_ratings_v3(
    ...
    train_loader = DataLoader(GameDatasetRatingsV3(train_recs), batch_size=args.batch_size,
                              shuffle=True, collate_fn=collate_ratings_v3)
    val_loader = DataLoader(GameDatasetRatingsV3(val_recs), batch_size=args.batch_size,
                            shuffle=False, collate_fn=collate_ratings_v3)
    test_loader = DataLoader(GameDatasetRatingsV3(test_recs), batch_size=args.batch_size,
                             shuffle=False, collate_fn=collate_ratings_v3)
    ...
    cfg = CmeRatingsV3Config(
        ...
        init_rating_track_slopes=tuple(args.init_rating_track_slopes),
        rating_mix_hidden=args.rating_mix_hidden,
        rating_mix_dropout=args.rating_mix_dropout,
        max_rating_mix_logit_delta=args.max_rating_mix_logit_delta,
    ...
    model = CmeRatingsV3(cfg).to(args.device)
    ...
    for name, param in model.named_parameters():
        ...
        if (name in calibration_param_names
                ...
                or name in {"rating_logit_bias", "log_rating_track_slopes", "rating_mix_base_logits", "raw_cme_gap_weight"}
