#!/usr/bin/env python3
"""Train MAN v3 — same matchup factorization as v2, but with an MLP head.

Drops the additive constraint that game outcome flows through margin = home_pts
- away_pts. Per-pair log_lambda is still trained via Poisson on observed matchup
rows. Game-level forward aggregates the 8x8 rotation grid into per-player
offense / defense expected-points features and feeds them to a small MLP that
predicts P(home_win) directly. Predicted margin is still emitted for inspection
but is only supervised if --margin-weight > 0 (off by default).
"""
...
class MANv3(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Head input: 4 per-player vectors of length K (off/def for both teams)
        # plus home_total, away_total, home_rest, away_rest
        head_input = 4 * rotation_size + 4
    ...
    def forward_game(self, batch: dict) -> dict:
        ...
        home_w = batch["home_w"]
        away_w = batch["away_w"]
        home_off_mask = ((home_w > 0).unsqueeze(2) & (away_w > 0).unsqueeze(1)).float()
        away_off_mask = ((away_w > 0).unsqueeze(2) & (home_w > 0).unsqueeze(1)).float()

        home_exp = (
            self.team_possessions
            * home_w.unsqueeze(2)
            * away_w.unsqueeze(1)
            * torch.exp(home_off_ll)
            * home_off_mask
        )
        away_exp = (
            self.team_possessions
            * away_w.unsqueeze(2)
            * home_w.unsqueeze(1)
            * torch.exp(away_off_ll)
            * away_off_mask
        )

        # Per-player aggregates
        home_off_per = home_exp.sum(dim=2)  # [B, K] — points home offensive player i scores
        away_def_per = home_exp.sum(dim=1)  # [B, K] — points away defender j allows
        away_off_per = away_exp.sum(dim=2)  # [B, K] — points away offensive player i scores
        home_def_per = away_exp.sum(dim=1)  # [B, K] — points home defender j allows

        home_total = home_off_per.sum(dim=1)
        away_total = away_off_per.sum(dim=1)
        ...
        feats = torch.cat(
            [
                ...
                away_off_per,
                away_def_per,
                home_total.unsqueeze(-1),
                away_total.unsqueeze(-1),
                batch["home_rest"].unsqueeze(-1),
                batch["away_rest"].unsqueeze(-1),
            ],
            dim=-1,
        )
        win_logit = self.head(feats).squeeze(-1)
...
def evaluate(
    model: MANv3, loader: DataLoader, device: torch.device
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            probs_all.append(torch.sigmoid(out["win_logit"]).cpu().numpy())
