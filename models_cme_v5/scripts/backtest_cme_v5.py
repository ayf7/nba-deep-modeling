#!/usr/bin/env python3
"""Expanding-window monthly backtest for CME-v5.
...
Defaults match `train_cme_v5.py`'s eleven-level loss
...
from cme_v5_model import CmeV5, CmeV5Config  # noqa: E402
from train_cme_v5 import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--calibration-hidden", type=int, default=64,
                   help="Hidden size for the v5 context-residual head.")
    p.add_argument("--calibration-dropout", type=float, default=0.05,
                   help="Dropout inside the v5 context-residual head.")
    ...
    p.add_argument("--season-calibration-hidden", type=int, default=16,
                   help="Hidden size for the v5 season-phase calibration head.")
    ...
    p.add_argument("--matchup-evidence-hidden", type=int, default=96,
                   help="Hidden size for the v5 pooled-lineup matchup evidence head.")
    p.add_argument("--max-matchup-evidence-residual", type=float, default=0.35,
                   help="Absolute tanh-bounded lineup/matchup evidence residual before uncertainty temperature.")
    p.add_argument("--uncertainty-hidden", type=int, default=16,
                   help="Hidden size for the v5 uncertainty-temperature head.")
    p.add_argument("--max-uncertainty-temperature-delta", type=float, default=0.75,
                   help="Maximum non-negative temperature increment; final logit is divided by 1+delta.")
    p.add_argument("--uncertainty-temperature-init-logit", type=float, default=-4.0,
                   help="Initial raw logit for uncertainty damping; -4 starts near neutral.")
    p.add_argument("--init-margin-sigma", type=float, default=13.0,
                   help="Warm start for the heteroscedastic margin sigma head.")
    ...
    # eleven-level loss weights (match train_cme_v5 defaults)
    ...
    p.add_argument("--margin-nll-w", type=float, default=0.02,
                   help="Small heteroscedastic margin NLL weight; trains the uncertainty signal used by v5.")
    p.add_argument("--calibration-reg-w", type=float, default=0.075,
                   help="L2 penalty weight on v5's tail-gated context residual.")
    p.add_argument("--matchup-evidence-reg-w", type=float, default=0.05,
                   help="L2 penalty weight on v5's pooled-lineup matchup evidence residual.")
    p.add_argument("--uncertainty-temp-reg-w", type=float, default=0.02,
                   help="L2 penalty weight on the non-negative uncertainty temperature increment.")
...
def predict_window(
    ...
    pretemp_probs: list[np.ndarray] = []
    season_logit_adjustments: list[np.ndarray] = []
    residual_tail_gates: list[np.ndarray] = []
    win_residuals: list[np.ndarray] = []
    matchup_evidence_residuals: list[np.ndarray] = []
    uncertainty_temperatures: list[np.ndarray] = []
    uncertainty_temperature_deltas: list[np.ndarray] = []
    margin_sigmas: list[np.ndarray] = []
    home_play_entropies: list[np.ndarray] = []
    away_play_entropies: list[np.ndarray] = []
    home_unavailable_probs: list[np.ndarray] = []
    away_unavailable_probs: list[np.ndarray] = []
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            pretemp_probs.append(torch.sigmoid(out["win_logit_pretemp"]).cpu().numpy())
            ...
            matchup_evidence_residuals.append(out["matchup_evidence_residual"].cpu().numpy())
            uncertainty_temperatures.append(out["uncertainty_temperature"].cpu().numpy())
            uncertainty_temperature_deltas.append(out["uncertainty_temperature_delta"].cpu().numpy())
            margin_sigmas.append(out["margin_sigma"].cpu().numpy())
            home_play_entropies.append(out["home_play_entropy"].cpu().numpy())
            away_play_entropies.append(out["away_play_entropy"].cpu().numpy())
            home_unavailable_probs.append(out["home_unavailable_prob"].cpu().numpy())
            away_unavailable_probs.append(out["away_unavailable_prob"].cpu().numpy())
    ...
    return (
        ...
        np.concatenate(pretemp_probs),
        np.concatenate(season_logit_adjustments),
        np.concatenate(residual_tail_gates),
        np.concatenate(win_residuals),
        np.concatenate(matchup_evidence_residuals),
        np.concatenate(uncertainty_temperatures),
        np.concatenate(uncertainty_temperature_deltas),
        np.concatenate(margin_sigmas),
        np.concatenate(home_play_entropies),
        np.concatenate(away_play_entropies),
        np.concatenate(home_unavailable_probs),
        np.concatenate(away_unavailable_probs),
...
def _build_ckpt_payload(model, cfg, vocab, team_vocab, box_weights, pair_weights,
                        ...
                        calibration_reg_w, matchup_evidence_reg_w, uncertainty_temp_reg_w,
                        season_calibration_reg_w, calibration_slope_reg_w, epoch):
    return {
        ...
        "loss_level_weights": {
            ...
            "matchup_evidence_reg": matchup_evidence_reg_w,
            "uncertainty_temp_reg": uncertainty_temp_reg_w,
...
def _build_optimizer(model: CmeV5, args) -> torch.optim.Optimizer:
    ...
    for name, param in model.named_parameters():
        ...
        if (name in calibration_param_names
                ...
                or name.startswith("matchup_evidence_head.")
                or name.startswith("uncertainty_temperature_head.")
                or name.startswith("margin_log_sigma_head.")):
...
def _inspect_player_predictions(
    ...
    loader = DataLoader(GameDatasetV5(sample), batch_size=len(sample),
                        shuffle=False, collate_fn=collate_v5)
    pts_i = BOX_INDEX["pts"]
    fgm_i = BOX_INDEX["fgm"]
    fga_i = BOX_INDEX["fga"]
...
def _track_player_predictions(
    ...
    loader = DataLoader(GameDatasetV5(test_recs), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_v5)
    model.eval()
...
def train_one_window(
...
) -> tuple[CmeV5, dict, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    cfg = CmeV5Config(
        ...
        matchup_evidence_hidden=args.matchup_evidence_hidden,
        max_matchup_evidence_residual=args.max_matchup_evidence_residual,
        uncertainty_hidden=args.uncertainty_hidden,
        max_uncertainty_temperature_delta=args.max_uncertainty_temperature_delta,
        uncertainty_temperature_init_logit=args.uncertainty_temperature_init_logit,
        init_margin_sigma=args.init_margin_sigma,
    ...
    if args.curriculum:
        ...
        p1_kwargs = dict(
            ...
            matchup_evidence_reg_w=args.matchup_evidence_reg_w,
            uncertainty_temp_reg_w=args.uncertainty_temp_reg_w,
        ...
        ckpt_matchup_evidence_reg_w = args.matchup_evidence_reg_w
        ckpt_uncertainty_temp_reg_w = args.uncertainty_temp_reg_w
        ...
        p2_kwargs = dict(
            ...
            matchup_evidence_reg_w=ckpt_matchup_evidence_reg_w,
            uncertainty_temp_reg_w=ckpt_uncertainty_temp_reg_w,
    ...
    if ckpt_dir is not None and args.save_final:
        final_payload = _build_ckpt_payload(
            ...
            ckpt_margin_nll_w, ckpt_calibration_reg_w, ckpt_matchup_evidence_reg_w, ckpt_uncertainty_temp_reg_w,
            ckpt_season_calibration_reg_w, ckpt_calibration_slope_reg_w, last_epoch,
        )
    else:
        final_payload = None
    ...
    if ckpt_dir is not None:
        ...
        torch.save(
            _build_ckpt_payload(
                ...
                ckpt_team_w, ckpt_player_w, ckpt_pair_w, ckpt_inv_w, ckpt_win_w,
                ckpt_margin_nll_w, ckpt_calibration_reg_w, ckpt_matchup_evidence_reg_w, ckpt_uncertainty_temp_reg_w,
                ckpt_season_calibration_reg_w, ckpt_calibration_slope_reg_w, best_epoch,
            ),
            ckpt_dir / "best.pt",
        )
    ...
    gids, probs, base_probs, affine_probs, temporal_probs, pretemp_probs, season_logit_adjustments, residual_tail_gates, win_residuals, matchup_evidence_residuals, uncertainty_temperatures, uncertainty_temperature_deltas, margin_sigmas, home_play_entropies, away_play_entropies, home_unavailable_probs, away_unavailable_probs, home_logit_biases, win_logit_slopes, h_pts, a_pts, margin_mu = predict_window(
    ...
    return model, window_info, gids, probs, base_probs, affine_probs, temporal_probs, pretemp_probs, season_logit_adjustments, residual_tail_gates, win_residuals, matchup_evidence_residuals, uncertainty_temperatures, uncertainty_temperature_deltas, margin_sigmas, home_play_entropies, away_play_entropies, home_unavailable_probs, away_unavailable_probs, home_logit_biases, win_logit_slopes, h_pts, a_pts, margin_mu
...
def main() -> None:
    ...
    if args.no_cyclic:
        ...
        _cv5c.TABULAR_FEATURE_COLUMNS = trimmed
    ...
    for wi, window_start in enumerate(windows):
        ...
        try:
            _model, info, gids, probs, base_probs, affine_probs, temporal_probs, pretemp_probs, season_logit_adjustments, residual_tail_gates, win_residuals, matchup_evidence_residuals, uncertainty_temperatures, uncertainty_temperature_deltas, margin_sigmas, home_play_entropies, away_play_entropies, home_unavailable_probs, away_unavailable_probs, home_logit_biases, win_logit_slopes, h_pts, a_pts, margin_mu = train_one_window(
        ...
        pretemp_probs_ordered = pretemp_probs[order]
        ...
        matchup_evidence_residuals_ordered = matchup_evidence_residuals[order]
        uncertainty_temperatures_ordered = uncertainty_temperatures[order]
        uncertainty_temperature_deltas_ordered = uncertainty_temperature_deltas[order]
        margin_sigmas_ordered = margin_sigmas[order]
        home_play_entropies_ordered = home_play_entropies[order]
        away_play_entropies_ordered = away_play_entropies[order]
        home_unavailable_probs_ordered = home_unavailable_probs[order]
        away_unavailable_probs_ordered = away_unavailable_probs[order]
        ...
        win_metrics = {
            ...
            "test_mean_pretemp_prob": float(np.mean(pretemp_probs_ordered)),
            ...
            "test_mean_matchup_evidence_residual": float(np.mean(matchup_evidence_residuals_ordered)),
            "test_mean_uncertainty_temperature": float(np.mean(uncertainty_temperatures_ordered)),
            "test_mean_uncertainty_temperature_delta": float(np.mean(uncertainty_temperature_deltas_ordered)),
            "test_mean_margin_sigma": float(np.mean(margin_sigmas_ordered)),
            "test_mean_home_play_entropy": float(np.mean(home_play_entropies_ordered)),
            "test_mean_away_play_entropy": float(np.mean(away_play_entropies_ordered)),
            "test_mean_home_unavailable_prob": float(np.mean(home_unavailable_probs_ordered)),
            "test_mean_away_unavailable_prob": float(np.mean(away_unavailable_probs_ordered)),
        ...
        print(f"[window {wi+1}/{len(windows)}] best_ep={info['best_epoch']:2d} "
              ...
              f"δ={np.mean(win_residuals_ordered):.3f} η={np.mean(matchup_evidence_residuals_ordered):.3f} "
              f"T={np.mean(uncertainty_temperatures_ordered):.3f} b={np.mean(home_logit_biases_ordered):.3f} "
              f"s={np.mean(win_logit_slopes_ordered):.3f} "
              f"({dt:.0f}s)")
        ...
        pred_df["pred_home_win_prob_pretemp"] = pretemp_probs_ordered
        ...
        pred_df["matchup_evidence_residual"] = matchup_evidence_residuals_ordered
        pred_df["uncertainty_temperature"] = uncertainty_temperatures_ordered
        pred_df["uncertainty_temperature_delta"] = uncertainty_temperature_deltas_ordered
        pred_df["margin_sigma"] = margin_sigmas_ordered
        pred_df["home_play_entropy"] = home_play_entropies_ordered
        pred_df["away_play_entropy"] = away_play_entropies_ordered
        pred_df["home_unavailable_prob"] = home_unavailable_probs_ordered
        pred_df["away_unavailable_prob"] = away_unavailable_probs_ordered
    ...
    pretemp_probs = predictions_df["pred_home_win_prob_pretemp"].to_numpy(dtype=float)
    ...
    matchup_evidence_residuals = predictions_df["matchup_evidence_residual"].to_numpy(dtype=float)
    uncertainty_temperatures = predictions_df["uncertainty_temperature"].to_numpy(dtype=float)
    uncertainty_temperature_deltas = predictions_df["uncertainty_temperature_delta"].to_numpy(dtype=float)
    margin_sigmas = predictions_df["margin_sigma"].to_numpy(dtype=float)
    home_play_entropies = predictions_df["home_play_entropy"].to_numpy(dtype=float)
    away_play_entropies = predictions_df["away_play_entropy"].to_numpy(dtype=float)
    home_unavailable_probs = predictions_df["home_unavailable_prob"].to_numpy(dtype=float)
    away_unavailable_probs = predictions_df["away_unavailable_prob"].to_numpy(dtype=float)
    ...
    overall = {
        ...
        "mean_pretemp_prob": float(pretemp_probs.mean()),
        ...
        "mean_matchup_evidence_residual": float(matchup_evidence_residuals.mean()),
        "mean_uncertainty_temperature": float(uncertainty_temperatures.mean()),
        "mean_uncertainty_temperature_delta": float(uncertainty_temperature_deltas.mean()),
        "mean_margin_sigma": float(margin_sigmas.mean()),
        "mean_home_play_entropy": float(home_play_entropies.mean()),
        "mean_away_play_entropy": float(away_play_entropies.mean()),
        "mean_home_unavailable_prob": float(home_unavailable_probs.mean()),
        "mean_away_unavailable_prob": float(away_unavailable_probs.mean()),
