#!/usr/bin/env python3
"""Train CME-v5 (hybrid Sinkhorn + K-channel + involvement-supervised).
...
from cme_v5_model import CmeV5, CmeV5Config, total_loss  # noqa: E402
...
def run_epoch(
    ...
    inv_w: float, win_w: float, margin_nll_w: float = 0.02,
    ...
    sum_L_matchup_evidence_reg = 0.0
    sum_L_uncertainty_temp_reg = 0.0
    ...
    sum_pretemp_prob = 0.0
    sum_label = 0.0
    sum_season_logit_adjustment = 0.0
    sum_residual_tail_gate = 0.0
    sum_win_residual = 0.0
    sum_matchup_evidence_residual = 0.0
    sum_uncertainty_temperature = 0.0
    sum_uncertainty_temperature_delta = 0.0
    sum_margin_sigma = 0.0
    sum_home_play_entropy = 0.0
    sum_away_play_entropy = 0.0
    sum_home_unavailable_prob = 0.0
    sum_away_unavailable_prob = 0.0
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(is_train):
            ...
            loss, diag = total_loss(
                ...
                matchup_evidence_reg_w=matchup_evidence_reg_w,
                uncertainty_temp_reg_w=uncertainty_temp_reg_w,
            ...
            with torch.no_grad():
                ...
                pretemp_probs = torch.sigmoid(out["win_logit_pretemp"])
        ...
        sum_L_matchup_evidence_reg += diag["L_matchup_evidence_reg"].item() * bs
        sum_L_uncertainty_temp_reg += diag["L_uncertainty_temp_reg"].item() * bs
        ...
        sum_pretemp_prob += pretemp_probs.mean().item() * bs
        ...
        sum_matchup_evidence_residual += out["matchup_evidence_residual"].mean().item() * bs
        sum_uncertainty_temperature += out["uncertainty_temperature"].mean().item() * bs
        sum_uncertainty_temperature_delta += out["uncertainty_temperature_delta"].mean().item() * bs
        sum_margin_sigma += out["margin_sigma"].mean().item() * bs
        sum_home_play_entropy += out["home_play_entropy"].mean().item() * bs
        sum_away_play_entropy += out["away_play_entropy"].mean().item() * bs
        sum_home_unavailable_prob += out["home_unavailable_prob"].mean().item() * bs
        sum_away_unavailable_prob += out["away_unavailable_prob"].mean().item() * bs
    ...
    return {
        ...
        "L_matchup_evidence_reg": sum_L_matchup_evidence_reg / total_n,
        "L_uncertainty_temp_reg": sum_L_uncertainty_temp_reg / total_n,
        ...
        "mean_pretemp_prob": sum_pretemp_prob / total_n,
        ...
        "mean_matchup_evidence_residual": sum_matchup_evidence_residual / total_n,
        "mean_uncertainty_temperature": sum_uncertainty_temperature / total_n,
        "mean_uncertainty_temperature_delta": sum_uncertainty_temperature_delta / total_n,
        "mean_margin_sigma": sum_margin_sigma / total_n,
        "mean_home_play_entropy": sum_home_play_entropy / total_n,
        "mean_away_play_entropy": sum_away_play_entropy / total_n,
        "mean_home_unavailable_prob": sum_home_unavailable_prob / total_n,
        "mean_away_unavailable_prob": sum_away_unavailable_prob / total_n,
...
def main() -> None:
    ...
    test_loader = DataLoader(GameDatasetV5(test_recs), batch_size=args.batch_size,
                             shuffle=False, collate_fn=collate_v5)
    ...
    print(f"[loss] team_w={args.team_w} player_w={args.player_w} "
          ...
          f"matchup_evidence_reg_w={args.matchup_evidence_reg_w} "
          f"uncertainty_temp_reg_w={args.uncertainty_temp_reg_w} "
        ...
        torch.save(
            {"model_state": model.state_dict(), "cfg": asdict(cfg),
             ...
             "loss_level_weights": {
                 ...
                 "matchup_evidence_reg": args.matchup_evidence_reg_w,
                 "uncertainty_temp_reg": args.uncertainty_temp_reg_w,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        row = {
            ...
            "train_L_matchup_evidence_reg": tr["L_matchup_evidence_reg"], "val_L_matchup_evidence_reg": va["L_matchup_evidence_reg"],
            "train_L_uncertainty_temp_reg": tr["L_uncertainty_temp_reg"], "val_L_uncertainty_temp_reg": va["L_uncertainty_temp_reg"],
            ...
            "train_mean_pretemp_prob": tr["mean_pretemp_prob"], "val_mean_pretemp_prob": va["mean_pretemp_prob"],
            ...
            "train_mean_matchup_evidence_residual": tr["mean_matchup_evidence_residual"], "val_mean_matchup_evidence_residual": va["mean_matchup_evidence_residual"],
            "train_mean_uncertainty_temperature": tr["mean_uncertainty_temperature"], "val_mean_uncertainty_temperature": va["mean_uncertainty_temperature"],
            "train_mean_uncertainty_temperature_delta": tr["mean_uncertainty_temperature_delta"], "val_mean_uncertainty_temperature_delta": va["mean_uncertainty_temperature_delta"],
            "train_mean_margin_sigma": tr["mean_margin_sigma"], "val_mean_margin_sigma": va["mean_margin_sigma"],
        ...
        print(
            ...
            f"η={tr['mean_matchup_evidence_residual']:.3f} T={tr['mean_uncertainty_temperature']:.3f} "
            ...
            f"va_η={va['mean_matchup_evidence_residual']:.3f} va_T={va['mean_uncertainty_temperature']:.3f} "
    ...
    print(
        ...
        f"η={te_best['mean_matchup_evidence_residual']:.3f} T={te_best['mean_uncertainty_temperature']:.3f} "
