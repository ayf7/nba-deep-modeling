#!/usr/bin/env python3
"""Expanding-window monthly backtest for CME-Availability-v2.

This mirrors the rolling-month evaluation used for CME-v4.2 while adding a
learned rotation/impact prior.  Each window trains only on games
...
from cme_availability_v2_common import (  # noqa: E402
    AVAILABILITY_FEATURE_DIM,
    MIN_ROTATION_TARGET_SECONDS,
    BOX_INDEX,
    ...
    GameDatasetAvailabilityV2,
    K_BOX,
    K_PAIR,
    PAIR_TARGETS,
    PLAYER_FORM_DIM,
    TABULAR_FEATURE_COLUMNS,
    build_records_availability_v2,
    build_team_game_player_seconds_index,
    build_team_vocab,
    build_vocab_from_records_availability_v2,
    collate_availability_v2,
...
from cme_availability_v2_model import CmeAvailabilityV2, CmeAvailabilityV2Config  # noqa: E402
from train_cme_availability_v2 import (  # noqa: E402
    default_box_weights,
    default_pair_weights,
    run_epoch,
)

DEFAULT_OUTPUT_ROOT = REPO_ROOT / "models_cme_availability_v2" / "artifacts"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--max-rotation-logit", type=float, default=2.0)
    p.add_argument("--max-minute-log-delta", type=float, default=1.5)
    p.add_argument("--rotation-log-prob-weight", type=float, default=0.50)
    ...
    p.add_argument("--availability-rotation-w", type=float, default=0.25)
...
def metrics_from_frame(df: pd.DataFrame) -> dict:
    ...
    optional = {
        ...
        "home_rotation_prob_mean": "mean_home_rotation_prob",
        "away_rotation_prob_mean": "mean_away_rotation_prob",
        "home_rotation_actual_rate": "mean_home_rotation_actual_rate",
        "away_rotation_actual_rate": "mean_away_rotation_actual_rate",
        "home_avail_play_prob_mean": "mean_home_avail_play_prob",
        "away_avail_play_prob_mean": "mean_away_avail_play_prob",
        "home_avail_play_prob_uncalibrated_mean": "mean_home_avail_play_prob_uncalibrated",
        "away_avail_play_prob_uncalibrated_mean": "mean_away_avail_play_prob_uncalibrated",
        "home_avail_cme_play_prob_mean": "mean_home_avail_cme_play_prob",
        "away_avail_cme_play_prob_mean": "mean_away_avail_cme_play_prob",
        ...
        "availability_play_prior_mix_strength": "mean_availability_play_prior_mix_strength",
        "availability_play_logit_bias": "mean_availability_play_logit_bias",
        "availability_play_logit_slope": "mean_availability_play_logit_slope",
    }
...
def build_optimizer(model: CmeAvailabilityV2, args: argparse.Namespace) -> torch.optim.Optimizer:
    calibration_param_names = {
        "home_logit_bias", "log_win_logit_slope", "global_scale",
        "availability_role_logit_strength",
    }
...
def epoch_kwargs(args: argparse.Namespace, box_weights: torch.Tensor, pair_weights: torch.Tensor) -> dict:
    return dict(
        ...
        availability_rotation_w=args.availability_rotation_w,
        ...
        availability_play_slope_reg_w_REMOVED=args.availability_play_slope_reg_w,
    )


def train_one_window(
    model: CmeAvailabilityV2,
    ...
    *,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        history.append({
            ...
            "train_availability_play_prior_mix_strength": tr["availability_play_prior_mix_strength"],
            "val_availability_play_prior_mix_strength": va["availability_play_prior_mix_strength"],
            "train_availability_play_logit_bias": tr["availability_play_logit_bias"],
            "val_availability_play_logit_bias": va["availability_play_logit_bias"],
            "train_availability_play_logit_slope": tr["availability_play_logit_slope"],
            "val_availability_play_logit_slope": va["availability_play_logit_slope"],
        })
        print(
...
def predict_records(
    ...
    *,
    ...
    model.eval()
    loader = DataLoader(
        GameDatasetAvailabilityV2(records, availability_stats),
        batch_size=batch_size,
        shuffle=False,
        collate_fn=collate_availability_v2,
    )
    rows: list[dict] = []
    record_i = 0
    with torch.no_grad():
        for batch in loader:
            ...
            play_mix_strength = float(out["availability_play_prior_mix_strength"].detach().cpu().item())
            play_cal_bias = float(out["availability_play_logit_bias"].detach().cpu().item())
            play_cal_slope = float(out["availability_play_logit_slope"].detach().cpu().item())
            ...
            h_prob_uncal_mean = ((out["home_avail_play_prob_uncalibrated"].detach().cpu() * h_mask).sum(dim=1) / h_denom).numpy()
            a_prob_uncal_mean = ((out["away_avail_play_prob_uncalibrated"].detach().cpu() * a_mask).sum(dim=1) / a_denom).numpy()
            h_cme_prob_mean = ((out["home_avail_cme_play_prob"].detach().cpu() * h_mask).sum(dim=1) / h_denom).numpy()
            a_cme_prob_mean = ((out["away_avail_cme_play_prob"].detach().cpu() * a_mask).sum(dim=1) / a_denom).numpy()
            ...
            rotation_log_threshold = math.log1p(MIN_ROTATION_TARGET_SECONDS)
            h_rotation_actual = (((batch["home_avail_log_seconds_actual"] >= rotation_log_threshold).to(torch.float32) * h_mask).sum(dim=1) / h_denom).numpy()
            a_rotation_actual = (((batch["away_avail_log_seconds_actual"] >= rotation_log_threshold).to(torch.float32) * a_mask).sum(dim=1) / a_denom).numpy()
            bs = len(probs)
            for j in range(bs):
                ...
                rows.append({
                    ...
                    "home_rotation_prob_mean": float(h_prob_mean[j]),
                    "away_rotation_prob_mean": float(a_prob_mean[j]),
                    "home_rotation_actual_rate": float(h_rotation_actual[j]),
                    "away_rotation_actual_rate": float(a_rotation_actual[j]),
                    "home_avail_play_prob_mean": float(h_prob_mean[j]),
                    "away_avail_play_prob_mean": float(a_prob_mean[j]),
                    "home_avail_play_prob_uncalibrated_mean": float(h_prob_uncal_mean[j]),
                    "away_avail_play_prob_uncalibrated_mean": float(a_prob_uncal_mean[j]),
                    "home_avail_cme_play_prob_mean": float(h_cme_prob_mean[j]),
                    "away_avail_cme_play_prob_mean": float(a_cme_prob_mean[j]),
                    ...
                    "availability_play_prior_mix_strength": play_mix_strength,
                    "availability_play_logit_bias": play_cal_bias,
                    "availability_play_logit_slope": play_cal_slope,
                })
            record_i += bs
...
def main() -> None:
    ...
    for w_i, window_start in enumerate(test_months, start=1):
        ...
        vocab = build_vocab_from_records_availability_v2(
        ...
        train_recs = build_records_availability_v2(train_df, matchup_rows=train_matchup, player_game_stats=train_pl, **common)
        val_recs = build_records_availability_v2(val_df, matchup_rows=val_matchup, player_game_stats=val_pl, **common)
        test_recs = build_records_availability_v2(test_df, matchup_rows=test_matchup, player_game_stats=test_pl, **common)
        ...
        train_loader = DataLoader(
            GameDatasetAvailabilityV2(train_recs, availability_stats),
            batch_size=args.batch_size,
            shuffle=True,
            collate_fn=collate_availability_v2,
        )
        val_loader = DataLoader(
            GameDatasetAvailabilityV2(val_recs, availability_stats),
            batch_size=args.batch_size,
            shuffle=False,
            collate_fn=collate_availability_v2,
        )
        ...
        cfg = CmeAvailabilityV2Config(
            ...
            max_rotation_logit=args.max_rotation_logit,
            max_minute_log_delta=args.max_minute_log_delta,
        rotation_log_prob_weight=args.rotation_log_prob_weight,
        ...
        model = CmeAvailabilityV2(cfg).to(args.device)
