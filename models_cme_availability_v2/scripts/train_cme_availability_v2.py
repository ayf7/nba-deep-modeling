#!/usr/bin/env python3
"""Train CME-Availability-v2 (hybrid Sinkhorn + K-channel + involvement-supervised).
...
from cme_availability_v2_common import (  # noqa: E402
    ...
    GameDatasetAvailabilityV2, build_records_availability_v2, build_team_vocab,
    build_vocab_from_records_availability_v2, collate_availability_v2,
...
from cme_availability_v2_model import CmeAvailabilityV2, CmeAvailabilityV2Config, total_loss_availability_v2  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--availability-hidden", type=int, default=64,
                   help="Hidden size for the learned rotation/impact head.")
    ...
    p.add_argument("--max-rotation-logit", type=float, default=2.0,
                   help="Absolute tanh-bounded meaningful-rotation logit.")
    ...
    p.add_argument("--rotation-log-prob-weight", type=float, default=0.50,
                   help="Weight of log rotation probability inside the rotation/impact role score.")
...
def run_epoch(
    model: CmeAvailabilityV2, loader: DataLoader, *, device: str,
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(is_train):
            out = model(batch)
            loss, diag = total_loss_availability_v2(
                out, batch,
                ...
                availability_rotation_w=availability_rotation_w,
                ...
                availability_play_slope_reg_w_REMOVED=availability_play_slope_reg_w_REMOVED,
            )
...
def main() -> None:
    ...
    train_loader = DataLoader(GameDatasetAvailabilityV2(train_recs, availability_stats), batch_size=args.batch_size,
                              shuffle=True, collate_fn=collate_availability_v2)
    val_loader = DataLoader(GameDatasetAvailabilityV2(val_recs, availability_stats), batch_size=args.batch_size,
                            shuffle=False, collate_fn=collate_availability_v2)
    test_loader = DataLoader(GameDatasetAvailabilityV2(test_recs, availability_stats), batch_size=args.batch_size,
                             shuffle=False, collate_fn=collate_availability_v2)
    ...
    print(f"[loss] team_w={args.team_w} player_w={args.player_w} "
          ...
          f"availability_rotation_w={args.availability_rotation_w} "
          ...
          f"availability_play_slope_reg_w_REMOVED={args.availability_play_slope_reg_w}")
        ...
        torch.save(
            {"model_state": model.state_dict(), "cfg": asdict(cfg),
             ...
             "loss_level_weights": {
                 ...
                 "availability_play": args.availability_rotation_w,
