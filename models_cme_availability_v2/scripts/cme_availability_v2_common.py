"""CME-Availability-v2 data layer.

This branch keeps the proven CME-v4.2 game/player/matchup construction, then
augments each pregame roster slot with leakage-safe rotation/impact features
and targets:

* meaningful-rotation target: did the player receive at least a small but
  material involvement-seconds threshold?
...
Unlike Availability-v1/v1.1, this data layer is **not** used to replace CME's
status-derived medical ``prob_play`` mask.  The downstream v2 model uses these
features only to shape a conservative rotation/impact prior inside CME's
...
# A player below this target is treated as outside the meaningful rotation.
MIN_ROTATION_TARGET_SECONDS = 300.0
...
@dataclass(frozen=True)
class GameRecordAvailabilityV2:
    base: GameRecordV42
...
def build_records_availability_v2(
    games: pd.DataFrame,
    histories: dict[str, list[TeamGameExposure]],
    *,
    vocab: Vocab,
    ...
    out: list[GameRecordAvailabilityV2] = []
    for base in base_records:
        ...
        out.append(GameRecordAvailabilityV2(
            base=base,
...
def fit_availability_feature_stats(
    records: list[GameRecordAvailabilityV2],
...
class GameDatasetAvailabilityV2(Dataset):
    def __init__(
        self,
        records: list[GameRecordAvailabilityV2],
...
def collate_availability_v2(batch: list[dict]) -> dict:
    base_batch = [
...
def build_vocab_from_records_availability_v2(*args, **kwargs):
...
__all__ = [
    ...
    "PlayerStatusDetail", "AvailabilityFeatureStats", "GameRecordAvailabilityV2",
    "build_team_game_player_seconds_index", "load_game_player_status_details",
    "build_availability_raw_features", "build_records_availability_v2",
    "fit_availability_feature_stats", "transform_availability_features",
    "GameDatasetAvailabilityV2", "collate_availability_v2",
