"""CME-v4: hybrid world model (Sinkhorn budget + K-channel play-style).
...
class CmeV4Config:
    ...
    # v4 calibration head: v3 derived win logits strictly from margin/scale,
    # which left no cheap way to correct aggregate home-side calibration drift.
    calibration_hidden: int = 64
    calibration_dropout: float = 0.05
    max_calibration_residual: float = 1.0
    init_home_logit_bias: float = 0.0
...
class CmeV4(nn.Module):
    def __init__(self, cfg: CmeV4Config) -> None:
        super().__init__()
        self.cfg = cfg
        d = cfg.d
        ...
        # -- margin -> win logit scale, with v4 calibration head --
        # v3 used win_logit = margin / fixed_scale exactly.  The v3 backtest
        # showed an aggregate home-probability shortfall, so v4 keeps the
        # interpretable margin-derived base logit but learns a small additive
        # correction: a global home/intercept term plus a bounded context
        # residual.  The residual head is zero-initialized, so a fresh v4
        # checkpoint starts behaviorally identical to v3.
    ...
    def forward(self, batch: dict) -> dict:
        ...
        if self.cfg.max_calibration_residual > 0.0:
            ...
            win_residual = torch.zeros_like(win_logit_base)
        win_logit = win_logit_base + self.home_logit_bias + win_residual
...
def calibration_residual_l2_loss(out: dict) -> torch.Tensor:
    """Keep the context-specific win-logit residual small by default.

    The global home intercept is intentionally *not* penalized here: the v3
    backtest failure mode was a persistent aggregate home-side calibration
    shortfall.  This term only discourages the residual MLP from replacing the
    world-model margin wholesale.
    """
...
def _smoke() -> None:
    ...
    print(f"\nloss = {loss.item():.4f}")
    for k in ("L_team", "L_player", "L_pair", "L_inv", "L_win", "L_margin_nll", "L_calibration_reg"):
