#!/usr/bin/env python3
"""Expanding-window monthly backtest for CME-v4.
...
Defaults match `train_cme_v4.py`'s seven-level loss
...
from cme_v4_model import CmeV4, CmeV4Config  # noqa: E402
from train_cme_v4 import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    # seven-level loss weights (match train_cme_v4 defaults)
...
def predict_window(
    model: CmeV4, records: list, batch_size: int, device: str,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
    loader = DataLoader(GameDatasetV4(records), batch_size=batch_size,
...
def _inspect_player_predictions(
    ...
    loader = DataLoader(GameDatasetV4(sample), batch_size=len(sample),
                        shuffle=False, collate_fn=collate_v4)
    pts_i = BOX_INDEX["pts"]
    fgm_i = BOX_INDEX["fgm"]
    fga_i = BOX_INDEX["fga"]
...
def _track_player_predictions(
    ...
    loader = DataLoader(GameDatasetV4(test_recs), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_v4)
    model.eval()
...
def train_one_window(
...
) -> tuple[CmeV4, dict, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    if ckpt_dir is not None and args.save_final:
        final_payload = _build_ckpt_payload(
            ...
            ckpt_margin_nll_w, ckpt_calibration_reg_w, last_epoch,
        )
    else:
        final_payload = None
    ...
    if ckpt_dir is not None:
        ...
        torch.save(
            _build_ckpt_payload(
                model, cfg, vocab, team_vocab, ckpt_box_weights, ckpt_pair_weights,
                ckpt_team_w, ckpt_player_w, ckpt_pair_w, ckpt_inv_w, ckpt_win_w,
                ckpt_margin_nll_w, ckpt_calibration_reg_w, best_epoch,
            ),
            ckpt_dir / "best.pt",
        )
    ...
    gids, probs, base_probs, win_residuals, home_logit_biases, h_pts, a_pts, margin_mu = predict_window(
    ...
    return model, window_info, gids, probs, base_probs, win_residuals, home_logit_biases, h_pts, a_pts, margin_mu
...
def main() -> None:
    ...
    for wi, window_start in enumerate(windows):
        ...
        try:
            _model, info, gids, probs, base_probs, win_residuals, home_logit_biases, h_pts, a_pts, margin_mu = train_one_window(
