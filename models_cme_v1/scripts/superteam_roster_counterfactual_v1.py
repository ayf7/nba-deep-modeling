#!/usr/bin/env python3
"""CME-v1 roster counterfactual: add a superteam to Lakers March 2026.
...
Mirrors the v2 counterfactual script; differences:
  * Uses CmeV1 (with home_points / away_points scalar outputs, no box vector).
  * Imports record/dataset/collate from man_xfmr_common (shared with v5).
"""
...
DEFAULT_CHECKPOINT = DEFAULT_ART / "backtest_v1_ckpt" / "windows" / "2026-03-01" / "best.pt"
DEFAULT_PLAYERS = (
    ...
    *,
    ...
    for batch in loader:
        ...
        data = {
            ...
            "pred_home_pts": home_pts.astype(float),
            "pred_away_pts": away_pts.astype(float),
        }
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--run-name", type=str, default="superteam_roster_cf_2026_03_v1backtest")
...
def plot_results(df: pd.DataFrame, out_path: Path, scenario_label: str) -> None:
    ...
    ax1.set_ylim(0, 1)
    ax1.set_title(f"CME-v1 Lakers March 2026 roster counterfactual: add {scenario_label}")
