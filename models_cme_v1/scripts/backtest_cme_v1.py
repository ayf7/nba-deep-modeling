#!/usr/bin/env python3
"""Expanding-window monthly backtest for CME-v1.

Same scaffold as v5 backtest (predictions.csv + window_metrics.csv per month),
swapping the model for CME-v1 and the loss for pair_poisson + margin_huber.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--run-name", type=str, default="backtest_cme_v1")
    ...
    p.add_argument("--eff-hidden", type=int, default=16)  # winner from sweep
...
def run_eval(model: CmeV1, loader: DataLoader, device: str,
             margin_coef: float, margin_delta: float) -> dict:
    model.eval()
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            marg = F.huber_loss(out["margin_mu"], batch["margin"], delta=margin_delta)
    ...
    return {
        ...
        "pois": sums["pois"] / n,
        "marg": sums["marg"] / n,
        "combined": sums["pois"] / n + margin_coef * sums["marg"] / n,
        "bce": sums["bce"] / n,
        "acc": sums["correct"] / n,
    }


def predict_probs(model: CmeV1, loader: DataLoader, device: str) -> np.ndarray:
    model.eval()
    probs: list[float] = []
    with torch.no_grad():
        for batch in loader:
...
def train_one_window(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            loss = pois + args.margin_coef * marg
            optim.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 5.0)
            optim.step()
        scheduler.step()
        va = run_eval(model, val_loader, device, args.margin_coef, args.margin_huber_delta)
        if va["combined"] < best_val - 1e-5:
            best_val = va["combined"]
            best_epoch = epoch
            best_bce = va["bce"]
...
def main() -> None:
    ...
    for w_idx, window_start in enumerate(windows, 1):
        ...
        row = {
            ...
            "best_val_combined": train_meta["best_val_combined"],
    ...
    if args.save_train_preds:
        train_preds = (pd.concat(all_train_preds, ignore_index=True)
                       if all_train_preds else pd.DataFrame())
        train_preds.to_csv(out_dir / "predictions_train.csv", index=False)
