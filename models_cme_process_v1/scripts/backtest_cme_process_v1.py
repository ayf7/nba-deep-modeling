#!/usr/bin/env python3
"""Expanding-window monthly backtest for Process-CME v1.
...
Defaults match `train_cme_process_v1.py`'s nine-level loss
...
from cme_process_v1_common import (  # noqa: E402
    BOX_INDEX, K_BOX, K_PAIR, K_PROCESS,
    ...
    DEFAULT_PROCESS_DB, PLAYER_FORM_DIM, PROCESS_PREGAME_FEATURE_COLUMNS,
    TABULAR_FEATURE_COLUMNS,
    GameDatasetProcessV1, build_records_process_v1, build_team_vocab,
    build_vocab_from_records_v42, collate_process_v1,
    fit_process_feature_stats, fit_process_target_stats, fit_tabular_stats,
...
from cme_process_v1_model import CmeProcessV1, CmeProcessV1Config  # noqa: E402
from train_cme_process_v1 import (  # noqa: E402
    default_box_weights, default_pair_weights, run_epoch,
)


DEFAULT_OUTPUT_ROOT = REPO_ROOT / "models_cme_process_v1" / "artifacts"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--process-db", type=Path, default=DEFAULT_PROCESS_DB,
                   help="Process artifact from data/scripts/build_game_process_features.py.")
    ...
    p.add_argument("--process-hidden", type=int, default=96,
                   help="Hidden width for process-state and process-residual heads.")
    p.add_argument("--process-dropout", type=float, default=0.05,
                   help="Dropout inside the process pathway.")
    p.add_argument("--max-process-logit-residual", type=float, default=0.25,
                   help="Absolute tanh-bounded process contribution to the win logit.")
    ...
    p.add_argument("--no-process-features", action="store_true",
                   help="Disable process-history pregame features while retaining process auxiliary targets/head.")
    p.add_argument("--no-cyclic", action="store_true",
                   help="Drop the 12 cyc_* columns from the tabular feature set (pre-cyclic baseline).")
    # nine-level loss weights (match train_cme_process_v1 defaults)
    ...
    p.add_argument("--process-w", type=float, default=0.25,
                   help="Auxiliary standardized process-target MSE weight.")
    p.add_argument("--process-residual-reg-w", type=float, default=0.05,
                   help="L2 regularization on the tail-gated process win-logit residual.")
...
def predict_window(
    model: CmeProcessV1, records: list, batch_size: int, device: str,
) -> tuple[np.ndarray, ...]:
    model.eval()
    loader = DataLoader(GameDatasetProcessV1(records), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_process_v1)
    ...
    process_logit_residuals: list[np.ndarray] = []
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            process_logit_residuals.append(out["process_logit_residual"].cpu().numpy())
    ...
    return (
        ...
        np.concatenate(process_logit_residuals),
...
def _build_ckpt_payload(model, cfg, vocab, team_vocab, box_weights, pair_weights,
                        ...
                        process_w, process_residual_reg_w,
                        process_feature_stats, process_target_stats, epoch):
    return {
        ...
        "process_feature_medians": process_feature_stats.medians.tolist(),
        "process_feature_means": process_feature_stats.means.tolist(),
        "process_feature_stds": process_feature_stats.stds.tolist(),
        "process_target_medians": process_target_stats.medians.tolist(),
        "process_target_means": process_target_stats.means.tolist(),
        "process_target_stds": process_target_stats.stds.tolist(),
        ...
        "loss_level_weights": {
            ...
            "process": process_w,
            "process_residual_reg": process_residual_reg_w,
        },
        "epoch": epoch,
    }


def _build_optimizer(model: CmeProcessV1, args) -> torch.optim.Optimizer:
    ...
    for name, param in model.named_parameters():
        ...
        if (name in calibration_param_names
                ...
                or name.startswith("process_win_head.")):
...
def _inspect_player_predictions(
    ...
    loader = DataLoader(GameDatasetProcessV1(sample), batch_size=len(sample),
                        shuffle=False, collate_fn=collate_process_v1)
    pts_i = BOX_INDEX["pts"]
    fgm_i = BOX_INDEX["fgm"]
    fga_i = BOX_INDEX["fga"]
...
def _track_player_predictions(
    ...
    loader = DataLoader(GameDatasetProcessV1(test_recs), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_process_v1)
    model.eval()
...
def train_one_window(
    ...
    process_features, process_targets,
    matchup_db: Path, core_db: Path,
    ckpt_dir: Path | None = None,
) -> tuple[CmeProcessV1, dict, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    process_feature_stats = fit_process_feature_stats(train_fit_df, process_features)
    process_target_stats = fit_process_target_stats(train_fit_df, process_targets)
    ...
    common = dict(
        ...
        game_odds=game_odds,
        process_feature_lookup=process_features,
        process_target_lookup=process_targets,
        process_feature_stats=process_feature_stats,
        process_target_stats=process_target_stats,
        use_process_features=not args.no_process_features,
    )
    train_recs = build_records_process_v1(
        train_fit_df, matchup_rows=train_matchup, player_game_stats=train_pl, **common,
    )
    val_recs = build_records_process_v1(
        val_df, matchup_rows=val_matchup, player_game_stats=val_pl, **common,
    )
    test_recs = build_records_process_v1(
        test_df, matchup_rows=test_matchup, player_game_stats=test_pl, **common,
    )

    train_loader = DataLoader(GameDatasetProcessV1(train_recs), batch_size=args.batch_size,
                              shuffle=True, collate_fn=collate_process_v1)
    val_loader = DataLoader(GameDatasetProcessV1(val_recs), batch_size=args.batch_size,
                            shuffle=False, collate_fn=collate_process_v1)

    import cme_process_v1_common as _cv4c
    tabular_dim = 0 if args.no_tabular else len(_cv4c.TABULAR_FEATURE_COLUMNS)
    if not args.no_process_features:
        tabular_dim += len(PROCESS_PREGAME_FEATURE_COLUMNS)
    cfg = CmeProcessV1Config(
        ...
        process_target_dim=K_PROCESS,
        process_hidden=args.process_hidden,
        process_dropout=args.process_dropout,
        max_process_logit_residual=args.max_process_logit_residual,
    )
    model = CmeProcessV1(cfg).to(args.device)
    ...
    if args.curriculum:
        ...
        p1_kwargs = dict(
            ...
            process_residual_reg_w=args.process_residual_reg_w,
        )
    ...
    if ckpt_dir is not None and args.save_final:
        final_payload = _build_ckpt_payload(
            ...
            args.process_w, args.process_residual_reg_w,
            process_feature_stats, process_target_stats, last_epoch,
        )
    else:
        final_payload = None
    ...
    if ckpt_dir is not None:
        ...
        torch.save(
            _build_ckpt_payload(
                ...
                ckpt_margin_nll_w, ckpt_calibration_reg_w, ckpt_season_calibration_reg_w, ckpt_calibration_slope_reg_w,
                args.process_w, args.process_residual_reg_w,
                process_feature_stats, process_target_stats, best_epoch,
            ),
            ckpt_dir / "best.pt",
        )
    ...
    gids, probs, base_probs, affine_probs, temporal_probs, season_logit_adjustments, residual_tail_gates, win_residuals, process_logit_residuals, home_logit_biases, win_logit_slopes, h_pts, a_pts, margin_mu = predict_window(
    ...
    return model, window_info, gids, probs, base_probs, affine_probs, temporal_probs, season_logit_adjustments, residual_tail_gates, win_residuals, process_logit_residuals, home_logit_biases, win_logit_slopes, h_pts, a_pts, margin_mu
...
def main() -> None:
    ...
    process_features = load_process_pregame_features(args.process_db, all_gids)
    process_targets = load_process_targets(args.process_db, all_gids)
    valid_process_targets = sum(int(v[1]) for v in process_targets.values())
    print(f"[load] n_games={len(games_all)} odds={len(game_odds)} "
          f"process_features={len(process_features)} valid_process_targets={valid_process_targets}")
    ...
    for wi, window_start in enumerate(windows):
        ...
        try:
            _model, info, gids, probs, base_probs, affine_probs, temporal_probs, season_logit_adjustments, residual_tail_gates, win_residuals, process_logit_residuals, home_logit_biases, win_logit_slopes, h_pts, a_pts, margin_mu = train_one_window(
                args, train_fit, val, test_block, histories, scores, statuses,
                calibration, game_odds, process_features, process_targets,
        ...
        process_logit_residuals_ordered = process_logit_residuals[order]
        ...
        win_metrics = {
            ...
            "test_mean_process_logit_residual": float(np.mean(process_logit_residuals_ordered)),
        ...
        print(f"[window {wi+1}/{len(windows)}] best_ep={info['best_epoch']:2d} "
              ...
              f"δ={np.mean(win_residuals_ordered):.3f} π={np.mean(process_logit_residuals_ordered):.3f} "
              f"b={np.mean(home_logit_biases_ordered):.3f} "
        ...
        pred_df["process_logit_residual"] = process_logit_residuals_ordered
    ...
    process_logit_residuals = predictions_df["process_logit_residual"].to_numpy(dtype=float)
    ...
    overall = {
        ...
        "mean_process_logit_residual": float(process_logit_residuals.mean()),
