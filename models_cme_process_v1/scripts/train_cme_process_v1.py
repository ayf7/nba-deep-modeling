#!/usr/bin/env python3
"""Train Process-CME v1 (hybrid Sinkhorn + K-channel + involvement-supervised).
...
from cme_process_v1_common import (  # noqa: E402
    BOX_INDEX, BOX_TARGETS, K_BOX, K_PAIR, K_PROCESS, PAIR_TARGETS,
    ...
    fit_player_form_stats, fit_process_feature_stats, fit_process_target_stats,
    fit_tabular_stats,
...
from cme_process_v1_model import CmeProcessV1, CmeProcessV1Config, total_loss_process_v1  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--no-process-features", action="store_true",
                   help="Disable process-history pregame features while keeping auxiliary process targets/head.")
...
def run_epoch(
    model: CmeProcessV1, loader: DataLoader, *, device: str,
    ...
    sum_L_process = 0.0
    sum_L_process_residual_reg = 0.0
    ...
    sum_process_logit_residual = 0.0
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(is_train):
            ...
            loss, diag = total_loss_process_v1(
                ...
                process_w=process_w,
                process_residual_reg_w=process_residual_reg_w,
            )
        ...
        sum_L_process += diag["L_process"].item() * bs
        sum_L_process_residual_reg += diag["L_process_residual_reg"].item() * bs
        ...
        sum_process_logit_residual += out["process_logit_residual"].mean().item() * bs
    ...
    return {
        ...
        "L_process": sum_L_process / total_n,
        "L_process_residual_reg": sum_L_process_residual_reg / total_n,
        ...
        "mean_process_logit_residual": sum_process_logit_residual / total_n,
...
def main() -> None:
    ...
    process_features = load_process_pregame_features(args.process_db, game_ids)
    process_targets = load_process_targets(args.process_db, game_ids)
    valid_process_targets = sum(int(v[1]) for v in process_targets.values())
    print(f"[load] process_features: {len(process_features)} / {len(game_ids)} games; "
          f"valid_process_targets: {valid_process_targets}")
    ...
    process_feature_stats = fit_process_feature_stats(train_df, process_features)
    process_target_stats = fit_process_target_stats(train_df, process_targets)
    ...
    test_loader = DataLoader(GameDatasetProcessV1(test_recs), batch_size=args.batch_size,
                             shuffle=False, collate_fn=collate_process_v1)
    ...
    print(f"[loss] team_w={args.team_w} player_w={args.player_w} "
          ...
          f"process_w={args.process_w} "
          f"process_residual_reg_w={args.process_residual_reg_w}")
        ...
        torch.save(
            {"model_state": model.state_dict(), "cfg": asdict(cfg),
             ...
             "loss_level_weights": {
                 ...
                 "process": args.process_w,
                 "process_residual_reg": args.process_residual_reg_w,
             },
             "epoch": epoch},
            path,
        )
