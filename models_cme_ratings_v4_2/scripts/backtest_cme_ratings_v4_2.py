#!/usr/bin/env python3
"""Expanding-window monthly backtest for CME-Ratings-v4.2.
...
from cme_ratings_v4_2_common import (  # noqa: E402
    ...
    DEFAULT_RATINGS_DB, DEFAULT_ROSTER_SHOCK_DB, RATING_FEATURE_COLUMNS, ROSTER_SHOCK_FEATURE_COLUMNS, K_ROSTER_SHOCK_FEATURES,
    PLAYER_FORM_DIM, TABULAR_FEATURE_COLUMNS,
    GameDatasetRatingsV4, build_records_ratings_v4, build_team_vocab,
    build_vocab_from_records_v42, collate_ratings_v4,
    fit_rating_feature_stats, fit_roster_shock_feature_stats, fit_tabular_stats,
    load_game_odds, load_game_player_status, load_game_scores, load_games,
    load_matchup_rows_v2, load_player_game_stats,
    load_rating_priors, load_roster_shock_features, load_status_calibration, load_team_exposures,
)
from cme_ratings_v4_2_model import CmeRatingsV42, CmeRatingsV42Config  # noqa: E402
from train_cme_ratings_v4_2 import (  # noqa: E402
    default_box_weights, default_pair_weights, run_epoch,
)


DEFAULT_OUTPUT_ROOT = REPO_ROOT / "models_cme_ratings_v4_2" / "artifacts"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--roster-shock-db", type=Path, default=DEFAULT_ROSTER_SHOCK_DB,
                   help="Pregame roster shock artifact from data/scripts/build_roster_shock_features.py.")
    ...
    p.add_argument("--roster-shock-hidden", type=int, default=32,
                   help="Hidden size for the roster-shock correction head.")
    p.add_argument("--roster-shock-dropout", type=float, default=0.05,
                   help="Dropout inside the roster-shock correction head.")
    p.add_argument("--max-roster-shock-signal-weight", type=float, default=1.25,
                   help="Absolute tanh bound on the signed roster-shock advantage scalar weight.")
    p.add_argument("--init-roster-shock-signal-weight", type=float, default=0.0,
                   help="Initialization for the bounded roster-shock advantage scalar weight.")
    p.add_argument("--max-roster-shock-feature-residual", type=float, default=0.45,
                   help="Absolute tanh bound for the learned roster-shock feature residual.")
    p.add_argument("--no-post-shock-calibration", action="store_true",
                   help="Disable the final post-shock affine calibration layer and use roster-adjusted ratings directly.")
    p.add_argument("--init-post-shock-logit-bias", type=float, default=0.0,
                   help="Initialization for the post-shock final calibration intercept.")
    p.add_argument("--init-post-shock-logit-slope", type=float, default=1.0,
                   help="Initialization for the positive post-shock final calibration slope.")
    ...
    p.add_argument("--no-roster-shock-features", action="store_true",
                   help="Disable standardized roster-shock feature vector while keeping the raw signed shock signal.")
    ...
    p.add_argument("--cme-feature-reg-w", type=float, default=0.0,
                   help="L2 penalty on the bounded CME feature residual added to the ratings anchor.")
    p.add_argument("--cme-gap-weight-reg-w", type=float, default=0.0,
    ...
    p.add_argument("--roster-shock-feature-reg-w", type=float, default=0.02,
                   help="L2 penalty on the bounded roster-shock neural feature residual.")
    p.add_argument("--roster-shock-signal-weight-reg-w", type=float, default=0.002,
                   help="L2 penalty on the bounded structured roster-shock advantage weight.")
    p.add_argument("--post-shock-bias-reg-w", type=float, default=0.0002,
                   help="Small L2 penalty on the post-shock calibration intercept.")
    p.add_argument("--post-shock-slope-reg-w", type=float, default=0.001,
                   help="Soft log-slope prior around 1.0 for the post-shock calibration slope.")
...
def predict_window(
    model: CmeRatingsV42, records: list, batch_size: int, device: str,
) -> tuple[np.ndarray, ...]:
    model.eval()
    loader = DataLoader(GameDatasetRatingsV4(records), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_ratings_v4)
    ...
    rating_logits_roster_adjusted: list[np.ndarray] = []
    rating_probs_roster_adjusted: list[np.ndarray] = []
    rating_logits_post_shock_calibrated: list[np.ndarray] = []
    rating_probs_post_shock_calibrated: list[np.ndarray] = []
    post_shock_logit_biases: list[np.ndarray] = []
    post_shock_logit_slopes: list[np.ndarray] = []
    roster_shock_signals: list[np.ndarray] = []
    roster_shock_signal_weights: list[np.ndarray] = []
    roster_shock_linear_adjustments: list[np.ndarray] = []
    roster_shock_feature_adjustments: list[np.ndarray] = []
    roster_shock_logit_adjustments: list[np.ndarray] = []
    has_roster_shocks: list[np.ndarray] = []
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            rating_logits_roster_adjusted.append(out["rating_logit_roster_adjusted"].cpu().numpy())
            rating_probs_roster_adjusted.append(out["rating_prob_roster_adjusted"].cpu().numpy())
            rating_logits_post_shock_calibrated.append(out["rating_logit_post_shock_calibrated"].cpu().numpy())
            rating_probs_post_shock_calibrated.append(out["rating_prob_post_shock_calibrated"].cpu().numpy())
            post_shock_logit_biases.append(np.full(probs_arr.shape, out["post_shock_logit_bias"].detach().cpu().item()))
            post_shock_logit_slopes.append(np.full(probs_arr.shape, out["post_shock_logit_slope"].detach().cpu().item()))
            roster_shock_signals.append(out["roster_shock_advantage_signal"].cpu().numpy())
            roster_shock_signal_weights.append(np.full(probs_arr.shape, out["roster_shock_signal_weight"].detach().cpu().item()))
            roster_shock_linear_adjustments.append(out["roster_shock_linear_adjustment"].cpu().numpy())
            roster_shock_feature_adjustments.append(out["roster_shock_feature_adjustment"].cpu().numpy())
            roster_shock_logit_adjustments.append(out["roster_shock_logit_adjustment"].cpu().numpy())
            has_roster_shocks.append(out["has_roster_shock"].cpu().numpy())
    ...
    return (
        ...
        np.concatenate(rating_logits_roster_adjusted),
        np.concatenate(rating_probs_roster_adjusted),
        np.concatenate(rating_logits_post_shock_calibrated),
        np.concatenate(rating_probs_post_shock_calibrated),
        np.concatenate(post_shock_logit_biases),
        np.concatenate(post_shock_logit_slopes),
        np.concatenate(roster_shock_signals),
        np.concatenate(roster_shock_signal_weights),
        np.concatenate(roster_shock_linear_adjustments),
        np.concatenate(roster_shock_feature_adjustments),
        np.concatenate(roster_shock_logit_adjustments),
        np.concatenate(has_roster_shocks),
...
def _build_ckpt_payload(model, cfg, vocab, team_vocab, box_weights, pair_weights,
                        ...
                        cme_feature_reg_w, cme_gap_weight_reg_w, rating_bias_reg_w, roster_shock_feature_reg_w, roster_shock_signal_weight_reg_w,
                        post_shock_bias_reg_w, post_shock_slope_reg_w,
                        rating_feature_stats, roster_shock_feature_stats, epoch):
    return {
        ...
        "roster_shock_feature_columns": list(ROSTER_SHOCK_FEATURE_COLUMNS),
        "roster_shock_feature_medians": (roster_shock_feature_stats.medians.tolist() if roster_shock_feature_stats is not None else None),
        "roster_shock_feature_means": (roster_shock_feature_stats.means.tolist() if roster_shock_feature_stats is not None else None),
        "roster_shock_feature_stds": (roster_shock_feature_stats.stds.tolist() if roster_shock_feature_stats is not None else None),
        "loss_level_weights": {
            ...
            "roster_shock_feature_reg": roster_shock_feature_reg_w,
            "roster_shock_signal_weight_reg": roster_shock_signal_weight_reg_w,
            "post_shock_bias_reg": post_shock_bias_reg_w,
            "post_shock_slope_reg": post_shock_slope_reg_w,
        },
        "epoch": epoch,
    }


def _build_optimizer(model: CmeRatingsV42, args) -> torch.optim.Optimizer:
    ...
    for name, param in model.named_parameters():
        ...
        if (name in calibration_param_names
                ...
                or name in {"rating_logit_bias", "log_rating_logit_slope", "raw_cme_gap_weight", "raw_roster_shock_signal_weight", "post_shock_logit_bias", "log_post_shock_logit_slope"}
                or name.startswith("cme_residual_head.")
                or name.startswith("roster_shock_head.")):
...
def _inspect_player_predictions(
    ...
    loader = DataLoader(GameDatasetRatingsV4(sample), batch_size=len(sample),
                        shuffle=False, collate_fn=collate_ratings_v4)
    pts_i = BOX_INDEX["pts"]
    fgm_i = BOX_INDEX["fgm"]
    fga_i = BOX_INDEX["fga"]
...
def _track_player_predictions(
    ...
    loader = DataLoader(GameDatasetRatingsV4(test_recs), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_ratings_v4)
    model.eval()
...
def train_one_window(
    ...
    roster_shock_feature_stats = fit_roster_shock_feature_stats(train_fit_df, roster_shocks)
    ...
    train_recs = build_records_ratings_v4(
        train_fit_df, matchup_rows=train_matchup, player_game_stats=train_pl,
        rating_lookup=rating_priors, rating_feature_stats=rating_feature_stats,
        roster_shock_lookup=roster_shocks, roster_shock_feature_stats=roster_shock_feature_stats,
        use_roster_shock_features=not args.no_roster_shock_features, **common,
    )
    val_recs = build_records_ratings_v4(
        val_df, matchup_rows=val_matchup, player_game_stats=val_pl,
        rating_lookup=rating_priors, rating_feature_stats=rating_feature_stats,
        roster_shock_lookup=roster_shocks, roster_shock_feature_stats=roster_shock_feature_stats,
        use_roster_shock_features=not args.no_roster_shock_features, **common,
    )
    test_recs = build_records_ratings_v4(
    ...
    train_loader = DataLoader(GameDatasetRatingsV4(train_recs), batch_size=args.batch_size,
                              shuffle=True, collate_fn=collate_ratings_v4)
    val_loader = DataLoader(GameDatasetRatingsV4(val_recs), batch_size=args.batch_size,
                            shuffle=False, collate_fn=collate_ratings_v4)

    import cme_v4_2_common as _cv4c
    tabular_dim = 0 if args.no_tabular else len(_cv4c.TABULAR_FEATURE_COLUMNS)
    cfg = CmeRatingsV42Config(
        ...
        roster_shock_feature_dim=0 if args.no_roster_shock_features else K_ROSTER_SHOCK_FEATURES,
        roster_shock_hidden=args.roster_shock_hidden,
        roster_shock_dropout=args.roster_shock_dropout,
        max_roster_shock_signal_weight=args.max_roster_shock_signal_weight,
        init_roster_shock_signal_weight=args.init_roster_shock_signal_weight,
        max_roster_shock_feature_residual=args.max_roster_shock_feature_residual,
        use_post_shock_calibration=not args.no_post_shock_calibration,
        init_post_shock_logit_bias=args.init_post_shock_logit_bias,
        init_post_shock_logit_slope=args.init_post_shock_logit_slope,
    )
    model = CmeRatingsV42(cfg).to(args.device)
    ...
    if args.curriculum:
        ...
        p1_kwargs = dict(
            ...
            roster_shock_feature_reg_w=args.roster_shock_feature_reg_w,
            roster_shock_signal_weight_reg_w=args.roster_shock_signal_weight_reg_w,
            post_shock_bias_reg_w=args.post_shock_bias_reg_w,
            post_shock_slope_reg_w=args.post_shock_slope_reg_w,
        )
        ...
        ckpt_roster_shock_feature_reg_w = args.roster_shock_feature_reg_w
        ckpt_roster_shock_signal_weight_reg_w = args.roster_shock_signal_weight_reg_w
        ckpt_post_shock_bias_reg_w = args.post_shock_bias_reg_w
        ckpt_post_shock_slope_reg_w = args.post_shock_slope_reg_w
    ...
    if ckpt_dir is not None and args.save_final:
        final_payload = _build_ckpt_payload(
            ...
            ckpt_cme_feature_reg_w, ckpt_cme_gap_weight_reg_w, args.rating_bias_reg_w, ckpt_roster_shock_feature_reg_w, ckpt_roster_shock_signal_weight_reg_w,
            ckpt_post_shock_bias_reg_w, ckpt_post_shock_slope_reg_w,
            rating_feature_stats, roster_shock_feature_stats, last_epoch,
        )
    else:
        final_payload = None
    ...
    if ckpt_dir is not None:
        ...
        torch.save(
            _build_ckpt_payload(
                ...
                ckpt_cme_feature_reg_w, ckpt_cme_gap_weight_reg_w, args.rating_bias_reg_w, ckpt_roster_shock_feature_reg_w, ckpt_roster_shock_signal_weight_reg_w,
                ckpt_post_shock_bias_reg_w, ckpt_post_shock_slope_reg_w,
                rating_feature_stats, roster_shock_feature_stats, best_epoch,
            ),
            ckpt_dir / "best.pt",
        )
    ...
    (gids, probs, base_probs, affine_probs, temporal_probs, season_logit_adjustments,
     ...
     rating_logits_roster_adjusted, rating_probs_roster_adjusted,
     rating_logits_post_shock_calibrated, rating_probs_post_shock_calibrated,
     post_shock_logit_biases, post_shock_logit_slopes, roster_shock_signals,
     roster_shock_signal_weights, roster_shock_linear_adjustments, roster_shock_feature_adjustments,
     roster_shock_logit_adjustments, has_roster_shocks,
...
def main() -> None:
    ...
    roster_shocks = load_roster_shock_features(args.roster_shock_db, all_gids)
    print(f"[load] n_games={len(games_all)} odds={len(game_odds)} ratings={len(rating_priors)} roster_shocks={len(roster_shocks)}")
    ...
    for wi, window_start in enumerate(windows):
        ...
        try:
            (_model, info, gids, probs, base_probs, affine_probs, temporal_probs,
             ...
             home_logit_biases, win_logit_slopes, h_pts, a_pts, margin_mu) = train_one_window(
                ...
                calibration, game_odds, rating_priors, roster_shocks, args.matchup_db, args.core_db,
                ckpt_dir=ckpt_dir,
            )
        ...
        rating_logits_roster_adjusted_ordered = rating_logits_roster_adjusted[order]
        rating_probs_roster_adjusted_ordered = rating_probs_roster_adjusted[order]
        rating_logits_post_shock_calibrated_ordered = rating_logits_post_shock_calibrated[order]
        rating_probs_post_shock_calibrated_ordered = rating_probs_post_shock_calibrated[order]
        post_shock_logit_biases_ordered = post_shock_logit_biases[order]
        post_shock_logit_slopes_ordered = post_shock_logit_slopes[order]
        roster_shock_signals_ordered = roster_shock_signals[order]
        roster_shock_signal_weights_ordered = roster_shock_signal_weights[order]
        roster_shock_linear_adjustments_ordered = roster_shock_linear_adjustments[order]
        roster_shock_feature_adjustments_ordered = roster_shock_feature_adjustments[order]
        roster_shock_logit_adjustments_ordered = roster_shock_logit_adjustments[order]
        has_roster_shocks_ordered = has_roster_shocks[order]
        ...
        win_metrics = {
            ...
            "test_mean_rating_logit_roster_adjusted": float(np.mean(rating_logits_roster_adjusted_ordered)),
            "test_mean_rating_prob_roster_adjusted": float(np.mean(rating_probs_roster_adjusted_ordered)),
            "test_mean_rating_logit_post_shock_calibrated": float(np.mean(rating_logits_post_shock_calibrated_ordered)),
            "test_mean_rating_prob_post_shock_calibrated": float(np.mean(rating_probs_post_shock_calibrated_ordered)),
            "test_mean_post_shock_logit_bias": float(np.mean(post_shock_logit_biases_ordered)),
            "test_mean_post_shock_logit_slope": float(np.mean(post_shock_logit_slopes_ordered)),
            "test_mean_roster_shock_signal": float(np.mean(roster_shock_signals_ordered)),
            "test_mean_roster_shock_signal_weight": float(np.mean(roster_shock_signal_weights_ordered)),
            "test_mean_roster_shock_linear_adjustment": float(np.mean(roster_shock_linear_adjustments_ordered)),
            "test_mean_roster_shock_feature_adjustment": float(np.mean(roster_shock_feature_adjustments_ordered)),
            "test_mean_roster_shock_logit_adjustment": float(np.mean(roster_shock_logit_adjustments_ordered)),
            "test_roster_shock_coverage": float(np.mean(has_roster_shocks_ordered)),
        ...
        print(f"[window {wi+1}/{len(windows)}] best_ep={info['best_epoch']:2d} "
              ...
              f"shock={np.mean(roster_shock_logit_adjustments_ordered):.3f} "
              f"post={np.mean(rating_probs_post_shock_calibrated_ordered):.3f} "
              f"psb={np.mean(post_shock_logit_biases_ordered):.3f} pss={np.mean(post_shock_logit_slopes_ordered):.3f} "
        ...
        pred_df["rating_logit_roster_adjusted"] = rating_logits_roster_adjusted_ordered
        pred_df["rating_home_win_prob_roster_adjusted"] = rating_probs_roster_adjusted_ordered
        pred_df["rating_logit_post_shock_calibrated"] = rating_logits_post_shock_calibrated_ordered
        pred_df["rating_home_win_prob_post_shock_calibrated"] = rating_probs_post_shock_calibrated_ordered
        pred_df["post_shock_logit_bias"] = post_shock_logit_biases_ordered
        pred_df["post_shock_logit_slope"] = post_shock_logit_slopes_ordered
        pred_df["roster_shock_advantage_signal"] = roster_shock_signals_ordered
        pred_df["roster_shock_signal_weight"] = roster_shock_signal_weights_ordered
        pred_df["roster_shock_linear_adjustment"] = roster_shock_linear_adjustments_ordered
        pred_df["roster_shock_feature_adjustment"] = roster_shock_feature_adjustments_ordered
        pred_df["roster_shock_logit_adjustment"] = roster_shock_logit_adjustments_ordered
        pred_df["has_roster_shock"] = has_roster_shocks_ordered
    ...
    rating_logits_roster_adjusted = predictions_df["rating_logit_roster_adjusted"].to_numpy(dtype=float)
    rating_probs_roster_adjusted = predictions_df["rating_home_win_prob_roster_adjusted"].to_numpy(dtype=float)
    rating_logits_post_shock_calibrated = predictions_df["rating_logit_post_shock_calibrated"].to_numpy(dtype=float)
    rating_probs_post_shock_calibrated = predictions_df["rating_home_win_prob_post_shock_calibrated"].to_numpy(dtype=float)
    post_shock_logit_biases = predictions_df["post_shock_logit_bias"].to_numpy(dtype=float)
    post_shock_logit_slopes = predictions_df["post_shock_logit_slope"].to_numpy(dtype=float)
    roster_shock_signals = predictions_df["roster_shock_advantage_signal"].to_numpy(dtype=float)
    roster_shock_signal_weights = predictions_df["roster_shock_signal_weight"].to_numpy(dtype=float)
    roster_shock_linear_adjustments = predictions_df["roster_shock_linear_adjustment"].to_numpy(dtype=float)
    roster_shock_feature_adjustments = predictions_df["roster_shock_feature_adjustment"].to_numpy(dtype=float)
    roster_shock_logit_adjustments = predictions_df["roster_shock_logit_adjustment"].to_numpy(dtype=float)
    has_roster_shocks = predictions_df["has_roster_shock"].to_numpy(dtype=float)
    ...
    rating_roster_bce = float(-np.mean(labels * np.log(np.clip(rating_probs_roster_adjusted, 1e-7, 1-1e-7))
                                      + (1 - labels) * np.log(np.clip(1 - rating_probs_roster_adjusted, 1e-7, 1-1e-7))))
    rating_roster_acc = float(np.mean((rating_probs_roster_adjusted > 0.5).astype(float) == labels))
    rating_roster_brier = float(np.mean((rating_probs_roster_adjusted - labels) ** 2))
    rating_post_bce = float(-np.mean(labels * np.log(np.clip(rating_probs_post_shock_calibrated, 1e-7, 1-1e-7))
                                      + (1 - labels) * np.log(np.clip(1 - rating_probs_post_shock_calibrated, 1e-7, 1-1e-7))))
    rating_post_acc = float(np.mean((rating_probs_post_shock_calibrated > 0.5).astype(float) == labels))
    rating_post_brier = float(np.mean((rating_probs_post_shock_calibrated - labels) ** 2))
    ...
    overall = {
        ...
        "rating_roster_adjusted_bce": rating_roster_bce,
        "rating_roster_adjusted_acc": rating_roster_acc,
        "rating_roster_adjusted_brier": rating_roster_brier,
        "rating_post_shock_calibrated_bce": rating_post_bce,
        "rating_post_shock_calibrated_acc": rating_post_acc,
        "rating_post_shock_calibrated_brier": rating_post_brier,
        ...
        "mean_rating_logit_roster_adjusted": float(rating_logits_roster_adjusted.mean()),
        "mean_rating_home_win_prob_roster_adjusted": float(rating_probs_roster_adjusted.mean()),
        "mean_rating_logit_post_shock_calibrated": float(rating_logits_post_shock_calibrated.mean()),
        "mean_rating_home_win_prob_post_shock_calibrated": float(rating_probs_post_shock_calibrated.mean()),
        "mean_post_shock_logit_bias": float(post_shock_logit_biases.mean()),
        "mean_post_shock_logit_slope": float(post_shock_logit_slopes.mean()),
