"""CME-Ratings-v4.2 data layer: Ratings-v2 + pregame roster-shock features.
...
questionable, or otherwise unavailable for each side before tip-off.  It also
adds an impact-aware channel derived from prior scoring + playmaking involvement
so the adjustment can distinguish high-leverage absences from lower-value
rotation losses. These features are standardized using the train window only,
while the signed ``roster_shock_advantage_signal`` is provided in raw bounded
form for a small structured logit adjustment.
"""
...
DEFAULT_ROSTER_SHOCK_DB = ARTIFACT_ROOT / "roster_shock_features_v4_2.sqlite"
...
ROSTER_SHOCK_FEATURE_COLUMNS: tuple[str, ...] = (
    ...
    "home_impact_mass_units",
    "away_impact_mass_units",
    ...
    "home_top1_unavailable_impact_share",
    "away_top1_unavailable_impact_share",
    "home_top3_unavailable_impact_share",
    "away_top3_unavailable_impact_share",
    "home_mean_minutes_to_tip",
    "away_mean_minutes_to_tip",
    "diff_unavailable_seconds_share",
    "diff_unavailable_points_share",
    "diff_unavailable_impact_share",
    "diff_out_seconds_share",
    "diff_uncertain_seconds_share",
    "diff_top1_unavailable_share",
    "diff_top3_unavailable_share",
    "diff_top1_unavailable_impact_share",
    "diff_top3_unavailable_impact_share",
...
def load_roster_shock_features(
    ...
    if not shock_db.exists():
        raise FileNotFoundError(
            f"Missing roster-shock DB: {shock_db}. Run data/scripts/build_roster_shock_features_v4_2.py first."
        )
