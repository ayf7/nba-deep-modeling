#!/usr/bin/env python3
"""Train CME-Ratings-v4.2 (roster-adjusted ratings final path + optional post-shock calibration).
...
from cme_ratings_v4_2_common import (  # noqa: E402
    BOX_INDEX, BOX_TARGETS, K_BOX, K_PAIR, K_RATING_FEATURES, PAIR_TARGETS,
    ...
    fit_player_form_stats, fit_rating_feature_stats, fit_roster_shock_feature_stats, fit_tabular_stats,
...
from cme_ratings_v4_2_model import CmeRatingsV42, CmeRatingsV42Config, total_loss  # noqa: E402
...
def run_epoch(
    model: CmeRatingsV42, loader: DataLoader, *, device: str,
    ...
    sum_L_cme_feature_reg = 0.0
    sum_L_cme_gap_weight_reg = 0.0
    sum_L_rating_bias_reg = 0.0
    ...
    sum_rating_margin_prior = 0.0
    sum_rating_logit_prior = 0.0
    sum_rating_prob_raw = 0.0
    sum_rating_logit_calibrated = 0.0
    sum_rating_prob_calibrated = 0.0
    sum_rating_prob_roster_adjusted = 0.0
    sum_rating_prob_post_shock_calibrated = 0.0
    sum_post_shock_logit_bias = 0.0
    sum_post_shock_logit_slope = 0.0
    sum_cme_gap = 0.0
    sum_disagreement_gate = 0.0
    sum_cme_gap_weight = 0.0
    sum_cme_gap_linear_residual = 0.0
    sum_cme_feature_residual = 0.0
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(is_train):
            ...
            loss, diag = total_loss(
                ...
                cme_feature_reg_w=cme_feature_reg_w,
                cme_gap_weight_reg_w=cme_gap_weight_reg_w,
                rating_bias_reg_w=rating_bias_reg_w,
                roster_shock_feature_reg_w=roster_shock_feature_reg_w,
                roster_shock_signal_weight_reg_w=roster_shock_signal_weight_reg_w,
                post_shock_bias_reg_w=post_shock_bias_reg_w,
                post_shock_slope_reg_w=post_shock_slope_reg_w,
            )
        ...
        sum_L_cme_feature_reg += diag["L_cme_feature_reg"].item() * bs
        sum_L_cme_gap_weight_reg += diag["L_cme_gap_weight_reg"].item() * bs
        sum_L_rating_bias_reg += diag["L_rating_bias_reg"].item() * bs
        ...
        sum_rating_margin_prior += out["rating_margin_prior"].mean().item() * bs
        sum_rating_logit_prior += out["rating_logit_prior"].mean().item() * bs
        sum_rating_prob_raw += out["rating_prob_raw"].mean().item() * bs
        sum_rating_logit_calibrated += out["rating_logit_calibrated"].mean().item() * bs
        sum_rating_prob_calibrated += out["rating_prob_calibrated"].mean().item() * bs
        sum_rating_prob_roster_adjusted += out["rating_prob_roster_adjusted"].mean().item() * bs
        sum_rating_prob_post_shock_calibrated += out["rating_prob_post_shock_calibrated"].mean().item() * bs
        sum_post_shock_logit_bias += out["post_shock_logit_bias"].detach().item() * bs
        sum_post_shock_logit_slope += out["post_shock_logit_slope"].detach().item() * bs
        sum_cme_gap += out["cme_gap"].mean().item() * bs
        sum_disagreement_gate += out["disagreement_gate"].mean().item() * bs
        sum_cme_gap_weight += out["cme_gap_weight"].detach().item() * bs
        sum_cme_gap_linear_residual += out["cme_gap_linear_residual"].mean().item() * bs
        sum_cme_feature_residual += out["cme_feature_residual"].mean().item() * bs
        correct += correct_bs
    ...
    return {
        ...
        "L_cme_feature_reg": sum_L_cme_feature_reg / total_n,
        "L_cme_gap_weight_reg": sum_L_cme_gap_weight_reg / total_n,
        "L_rating_bias_reg": sum_L_rating_bias_reg / total_n,
        ...
        "mean_rating_margin_prior": sum_rating_margin_prior / total_n,
        "mean_rating_logit_prior": sum_rating_logit_prior / total_n,
        "mean_rating_prob_raw": sum_rating_prob_raw / total_n,
        "mean_rating_logit_calibrated": sum_rating_logit_calibrated / total_n,
        "mean_rating_prob_calibrated": sum_rating_prob_calibrated / total_n,
        "mean_rating_prob_roster_adjusted": sum_rating_prob_roster_adjusted / total_n,
        "mean_rating_prob_post_shock_calibrated": sum_rating_prob_post_shock_calibrated / total_n,
        "post_shock_logit_bias": sum_post_shock_logit_bias / total_n,
        "post_shock_logit_slope": sum_post_shock_logit_slope / total_n,
        "mean_cme_gap": sum_cme_gap / total_n,
        "mean_disagreement_gate": sum_disagreement_gate / total_n,
        "cme_gap_weight": sum_cme_gap_weight / total_n,
        "mean_cme_gap_linear_residual": sum_cme_gap_linear_residual / total_n,
        "mean_cme_feature_residual": sum_cme_feature_residual / total_n,
        "alpha_off_corr": (
...
def main() -> None:
    ...
    rating_priors = load_rating_priors(args.ratings_db, game_ids)
    print(f"[load] rating_priors: {len(rating_priors)} / {len(game_ids)} games")
    roster_shocks = load_roster_shock_features(args.roster_shock_db, game_ids)
    print(f"[load] roster_shocks: {len(roster_shocks)} / {len(game_ids)} games")
    ...
    rating_feature_stats = fit_rating_feature_stats(train_df, rating_priors)
    roster_shock_feature_stats = fit_roster_shock_feature_stats(train_df, roster_shocks)
    ...
    train_recs = build_records_ratings_v4(
        train_df, matchup_rows=train_matchup, player_game_stats=train_pl,
    ...
    test_loader = DataLoader(GameDatasetRatingsV4(test_recs), batch_size=args.batch_size,
                             shuffle=False, collate_fn=collate_ratings_v4)
    ...
    print(f"[loss] team_w={args.team_w} player_w={args.player_w} "
          ...
          f"cme_feature_reg_w={args.cme_feature_reg_w} "
          f"cme_gap_weight_reg_w={args.cme_gap_weight_reg_w} "
          f"rating_bias_reg_w={args.rating_bias_reg_w} "
          f"roster_shock_feature_reg_w={args.roster_shock_feature_reg_w} "
          f"roster_shock_signal_weight_reg_w={args.roster_shock_signal_weight_reg_w} "
          f"post_shock_bias_reg_w={args.post_shock_bias_reg_w} "
          f"post_shock_slope_reg_w={args.post_shock_slope_reg_w}")
    ...
    def save_ckpt(path: Path, epoch: int) -> None:
        torch.save(
            {"model_state": model.state_dict(), "cfg": asdict(cfg),
             ...
             "rating_feature_medians": rating_feature_stats.medians.tolist(),
             "rating_feature_means": rating_feature_stats.means.tolist(),
             "rating_feature_stds": rating_feature_stats.stds.tolist(),
             "roster_shock_feature_columns": list(ROSTER_SHOCK_FEATURE_COLUMNS),
             "roster_shock_feature_medians": roster_shock_feature_stats.medians.tolist(),
             "roster_shock_feature_means": roster_shock_feature_stats.means.tolist(),
             "roster_shock_feature_stds": roster_shock_feature_stats.stds.tolist(),
             "loss_level_weights": {
                 ...
                 "cme_feature_reg": args.cme_feature_reg_w,
                 "cme_gap_weight_reg": args.cme_gap_weight_reg_w,
                 "rating_bias_reg": args.rating_bias_reg_w,
                 "roster_shock_feature_reg": args.roster_shock_feature_reg_w,
                 "roster_shock_signal_weight_reg": args.roster_shock_signal_weight_reg_w,
                 "post_shock_bias_reg": args.post_shock_bias_reg_w,
                 "post_shock_slope_reg": args.post_shock_slope_reg_w,
             },
             "epoch": epoch},
            path,
        )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        row = {
            ...
            "train_mean_rating_margin_prior": tr["mean_rating_margin_prior"], "val_mean_rating_margin_prior": va["mean_rating_margin_prior"],
            "train_mean_rating_prob_calibrated": tr["mean_rating_prob_calibrated"], "val_mean_rating_prob_calibrated": va["mean_rating_prob_calibrated"],
            "train_mean_rating_prob_roster_adjusted": tr["mean_rating_prob_roster_adjusted"], "val_mean_rating_prob_roster_adjusted": va["mean_rating_prob_roster_adjusted"],
            "train_mean_rating_prob_post_shock_calibrated": tr["mean_rating_prob_post_shock_calibrated"], "val_mean_rating_prob_post_shock_calibrated": va["mean_rating_prob_post_shock_calibrated"],
            "train_post_shock_logit_bias": tr["post_shock_logit_bias"], "val_post_shock_logit_bias": va["post_shock_logit_bias"],
            "train_post_shock_logit_slope": tr["post_shock_logit_slope"], "val_post_shock_logit_slope": va["post_shock_logit_slope"],
            "train_mean_disagreement_gate": tr["mean_disagreement_gate"], "val_mean_disagreement_gate": va["mean_disagreement_gate"],
            "train_cme_gap_weight": tr["cme_gap_weight"], "val_cme_gap_weight": va["cme_gap_weight"],
        }
        history.append(row)
        extra = ""
        ...
        print(
            ...
            f"rM={tr['mean_rating_margin_prior']:.2f} rP={tr['mean_rating_prob_calibrated']:.3f} "
            f"gate={tr['mean_disagreement_gate']:.3f} gw={tr['cme_gap_weight']:.3f} | "
            ...
            f"va_b={va['home_logit_bias']:.3f} va_s={va['win_logit_slope']:.3f} "
            f"va_rM={va['mean_rating_margin_prior']:.2f} va_rP={va['mean_rating_prob_calibrated']:.3f} "
            f"va_gate={va['mean_disagreement_gate']:.3f} va_gw={va['cme_gap_weight']:.3f}{extra} "
    ...
    print(
        ...
        f"rM={te_best['mean_rating_margin_prior']:.2f} rP={te_best['mean_rating_prob_calibrated']:.3f} "
        f"gate={te_best['mean_disagreement_gate']:.3f} gw={te_best['cme_gap_weight']:.3f}"
    )
