...
from cme_v4_2_common import (  # noqa: E402
    ...
    GameDatasetV42, build_records_v42, build_team_vocab,
    build_vocab_from_records_v42, collate_v42,
    fit_tabular_stats,
...
from cme_v4_2_model import CmeV42, CmeV42Config  # noqa: E402
from train_cme_v4_2 import (  # noqa: E402
    default_box_weights, default_pair_weights, run_epoch,
)


DEFAULT_OUTPUT_ROOT = REPO_ROOT / "models_cme_v4_2" / "artifacts"
...
def predict_window(
    model: CmeV42, records: list, batch_size: int, device: str,
) -> tuple[np.ndarray, ...]:
    model.eval()
    loader = DataLoader(GameDatasetV42(records), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_v42)
...
def _build_optimizer(model: CmeV42, args) -> torch.optim.Optimizer:
...
def _inspect_player_predictions(
    ...
    loader = DataLoader(GameDatasetV42(sample), batch_size=len(sample),
                        shuffle=False, collate_fn=collate_v42)
    pts_i = BOX_INDEX["pts"]
    fgm_i = BOX_INDEX["fgm"]
    fga_i = BOX_INDEX["fga"]
...
def _track_player_predictions(
    ...
    loader = DataLoader(GameDatasetV42(test_recs), batch_size=batch_size,
                        shuffle=False, collate_fn=collate_v42)
    model.eval()
...
def train_one_window(
...
) -> tuple[CmeV42, dict, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    train_loader = DataLoader(GameDatasetV42(train_recs), batch_size=args.batch_size,
                              shuffle=True, collate_fn=collate_v42)
    val_loader = DataLoader(GameDatasetV42(val_recs), batch_size=args.batch_size,
                            shuffle=False, collate_fn=collate_v42)
    ...
    model = CmeV42(cfg).to(args.device)
