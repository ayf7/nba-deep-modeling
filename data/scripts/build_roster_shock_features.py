...
DEFAULT_OUTPUT = ARTIFACT_ROOT / "roster_shock_features.sqlite"
...
def _read_exposures(matchup_db: Path) -> dict[str, dict[str, dict[str, tuple[float, float]]]]:
    ...
    with sqlite3.connect(matchup_db) as conn:
        df = pd.read_sql_query(query, conn)
    out: dict[str, dict[str, dict[str, tuple[float, float]]]] = defaultdict(lambda: defaultdict(dict))
    for row in df.itertuples(index=False):
        out[str(row.game_id)][str(row.team_id)][str(row.player_id)] = (
            float(row.exposure_seconds or 0.0),
            float(row.player_points or 0.0),
        )
    return out
...
def _update_team_state(
    ...
    *,
    ...
    all_pids = set(sec_map) | set(pts_map) | set(exposures)
    keep_sec: dict[str, float] = {}
    keep_pts: dict[str, float] = {}
    alpha = 1.0 - float(decay)
    for pid in all_pids:
        cur_sec, cur_pts = exposures.get(pid, (0.0, 0.0))
...
def build_rows(
    games: pd.DataFrame,
    exposures: dict[str, dict[str, dict[str, tuple[float, float]]]],
    ...
    *,
    ...
    for game in games.itertuples(index=False):
        ...
        h = _summarize_team(home, avail_game.get(home, []), seconds_state, points_state, games_seen)
        a = _summarize_team(away, avail_game.get(away, []), seconds_state, points_state, games_seen)
        ...
        # Signed scalar for the structured v4 linear shock path.  Positive =>
        # away more roster-shocked than home => home team should improve.
        advantage_signal = (
        ...
        _update_team_state(home, exposure_game.get(home, {}), seconds_state, points_state, games_seen, decay=ema_decay)
        _update_team_state(away, exposure_game.get(away, {}), seconds_state, points_state, games_seen, decay=ema_decay)
...
def main() -> None:
    ...
    print(f"[roster-shock] games={len(rows)} output={args.output}")
    if not rows.empty:
        print("[roster-shock] date_range=", rows["game_date"].min(), "..", rows["game_date"].max())
        print("[roster-shock] report_coverage=", round(float(rows["has_roster_shock_report"].mean()), 4))
        print("[roster-shock] mean_advantage_signal=", round(float(rows["roster_shock_advantage_signal"].mean()), 6))
