#!/usr/bin/env python3
"""Train CME-Availability-v1 (hybrid Sinkhorn + K-channel + involvement-supervised).
...
from cme_availability_v1_common import (  # noqa: E402
    ...
    GameDatasetAvailabilityV1, build_records_availability_v1, build_team_vocab,
    build_vocab_from_records_availability_v1, collate_availability_v1,
...
from cme_availability_v1_model import CmeAvailabilityV1, CmeAvailabilityV1Config, total_loss_availability_v1  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--max-play-logit-delta", type=float, default=3.0,
                   help="Bounded learned correction around the status-only play-probability prior.")
    ...
    p.add_argument("--init-role-prior-strength", type=float, default=0.25,
                   help="Initial bounded strength of the predicted minutes prior inside the involvement softmax.")
    # loss weights
    ...
    p.add_argument("--availability-play-w", type=float, default=0.50,
                   help="Weight on player pregame appearance BCE.")
...
def run_epoch(
    model: CmeAvailabilityV1, loader: DataLoader, *, device: str,
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(is_train):
            out = model(batch)
            loss, diag = total_loss_availability_v1(
                out, batch,
...
def main() -> None:
    ...
    train_loader = DataLoader(GameDatasetAvailabilityV1(train_recs, availability_stats), batch_size=args.batch_size,
                              shuffle=True, collate_fn=collate_availability_v1)
    val_loader = DataLoader(GameDatasetAvailabilityV1(val_recs, availability_stats), batch_size=args.batch_size,
                            shuffle=False, collate_fn=collate_availability_v1)
    test_loader = DataLoader(GameDatasetAvailabilityV1(test_recs, availability_stats), batch_size=args.batch_size,
                             shuffle=False, collate_fn=collate_availability_v1)
