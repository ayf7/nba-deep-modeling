"""CME-v3: hybrid world model (Sinkhorn budget + K-channel play-style).
...
Forward pipeline per game:
 ...
 10. Assemble per-player BOX (14-d) -> team BOX -> margin -> win_logit.
...
class CmeV3(nn.Module):
    def __init__(self, cfg: CmeV3Config) -> None:
        super().__init__()
        self.cfg = cfg
        d = cfg.d
        ...
        # -- margin -> win_logit scale + heteroscedastic sigma head --
...
def _smoke() -> None:
    ...
    print(f"\nloss = {loss.item():.4f}")
    for k in ("L_team", "L_player", "L_pair", "L_inv", "L_win", "L_margin_nll"):
